...
void BP_Component::do_task(void)
{
    ...
    switch (_task_counter) {
        ...
        case 9:
            ...
            if (!_flags.vehicle_has_passed_prearm_checks)
                if (copter.letme_get_pream_checks_passed() && _flags.gcs_has_been_detected) {
                    copter.letme_gcs_send_text(MAV_SEVERITY_INFO, "Vehicle passed prearm checks");
                    _flags.vehicle_has_passed_prearm_checks = true;
                }
            if (!_flags.vehicle_has_been_armed)
                if (copter.letme_get_motors_armed() && _flags.gcs_has_been_detected) {
