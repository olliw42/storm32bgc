...
const AP_Param::GroupInfo BP_Component::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("_RC_TRGT", 0, BP_Component, _rc_target_type, 1),
...
/// friend of BP_Component
// preforms the prearm check, and sedns out message if needed, returns true if passed
bool BP_Component_pre_arm_check(bool display_failure)
{
    if (_bp.myself){
        ...
        if (display_failure) {
            copter.letme_gcs_send_text(MAV_SEVERITY_CRITICAL, "PreArm: STorM32 not yet in NORMAL state");
        }
    }
...
/// friend of BP_Component
// preforms the arm check, and sedns out message if needed, returns true if passed
bool BP_Component_arm_check(bool display_failure)
{
    if (_bp.myself){
        ...
        if (display_failure) {
            copter.letme_gcs_send_text(MAV_SEVERITY_CRITICAL, "Arm: STorM32 not yet in NORMAL state");
        }
    }
    ...
    switch (_task_counter) {
        case 1:case 6:
            ...
            if (_bp.do_mount_control_is_set) {
                _bp.do_mount_control_is_set = false;
                handle_do_mount_control_is_set();
            }
            break;
        case 2:case 7:
        ...
        case 9:
            // send stuff to GCS
            // inform the GCS, as soon as a GCS is connected
            ...
            if ((!_flags.mount_has_been_armed) && _flags.mount_is_armed && _flags.gcs_has_been_detected) {
...
{
    if (_bp.mount_control_mode <= MAV_MOUNT_MODE_NEUTRAL) {
        if (_mount_mode_last == _bp.mount_control_mode) {
            ...
            // teh camera does not need to be recentered explicitly, thus break;
