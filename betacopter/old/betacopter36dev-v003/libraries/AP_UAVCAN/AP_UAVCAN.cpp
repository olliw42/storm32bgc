...
//OW
                    ...
                    storm32_nodespecific_out_array[_uavcan_i]->setPriority(uavcan::TransferPriority::MiddleLower); //OneHigherThanLowest);
//OWEND
...
void AP_UAVCAN::storm32_nodespecific_send(uint8_t* payload, uint8_t payload_len)
{
...
void AP_UAVCAN::storm32_do_cyclic(uint64_t current_time_ms)
{
    ...
    if( _storm32_out.nodespecific_to_send ){
        if (storm32_nodespecific_out_sem_take()) {
            ...
            return; //always only send one message per cycle
        }
    }
}
