...
my $VersionStr= '10. Jan. 2015 v0.90';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '89' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()

my @FirmwareVersionList= ( 'v0.90','v0.90 NT','v0.89e','v0.89e NT', 'v0.87e','v0.87e NT','v0.86e','v0.86e NT','v0.80' ); #versions available in the flash tab selector

my $UpdateInstructionsStr =
"When you're updating from v0.89e then you just need to flash the new firmware. ".
"ALL settings, including the scripts and calibration data, are copied over.

For all older versions, this firmware will reset all parameter and script settings when flashed. ".
"Please memorize your old settings before flashing the firmware, and restore them afterwards manually. ".
"The calibration data is not affected and will be copied over correctly.

The NT Logger firmware got updated too, please flash it if you're using one.

Please note that, for reasons of size, the NTLoggerTool is NOT included in the standard package! ".
"If you need it please download the respective firmware package from the usual places.";
...
my @STorM32BGCBoardList= (
{
  ...
  uc => 'F103T8',
  hexfile => 'o323bgc_ntimu_v012_module_v10_f103t8',
},{
  name => 'NT Motor Module v1.0',
  uc => 'F103T8',
  hexfile => 'o323bgc_ntmotor_v012_module_v10_f103t8',
},{
  name => 'NT Logger Module v1.0',
  uc => 'F103T8',
  hexfile => 'o323bgc_ntlogger_v015_module_v10_f103t8',
},{
  name => 'NT Imu Module CC3D Atom',
  uc => 'F103T8',
  hexfile => 'o323bgc_ntimu_v013_module_cc3datom_f103cb',
}
);
...
##--- EXPERT tab  --------------------
  ...
  size => 1,
  adr => 77,
  choices => [ 'off', 'basic', 'basic + accgyro', 'basic + pid', 'basic + accgyro raw', 'basic + accgyro + pid', 'full' ],
...
# NT
#---------------------------
my $NTDataLoggerToolFile = "NTLoggerTool_v005.exe";
if( defined $IniFile ){
  if( defined $IniFile->val('NT','NTDataLoggerTool') ){ $NTDataLoggerToolFile= $IniFile->val('NT','NTDataLoggerTool'); }
}
#if( not(-f $NTDataLoggerToolFile) ){
#  $NTDataLoggerToolFile = ''; #this is to indicate that the file is not existing  BUG: This writes '' into IniFile!!!
#}
...
my @NTDataLoggerTool = ( ">NT Data Logger Tool... ", 't_DataLoggerTool', );
...
if( not(-f $NTDataLoggerToolFile) ){ @NTDataLoggerTool = (); }
...
be in the range of -7000 to +7000, and the Scale values in the
range of +15000 to +17500.
...
sub m_UpdateNotes_Click{
  ...
  "RELEASE NOTES:\n------------------------------------------------------------\n".
  $ReleaseNotesStr."\n\n".
  "\nComment: This message will not be shown again."."\n\n"
  ,
...
#    $w_Main->ShellExecute('open',"$BGCToolRunFile.bat","",'',1);
  TextOut( "\n".'NT DataLogger Tool started'."\n" );
  $w_Main->ShellExecute('open',$NTDataLoggerToolFile,"",'',1);
  1;
}
...
sub ExtractStatusInfo{
  ...
  if( CheckStatus($status,$STATUS_IMU_PRESENT) ){
    ...
    if( CheckStatus($status,$STATUS_NTBUS_INUSE) ){
      ...
      if( CheckStatus($status2,$STATUS2_IMU_8000) ){ $sa .= ' (ES)'; }
    }
  ...
  if( CheckStatus($status,$STATUS_IMU2_PRESENT) ){
    ...
    if( CheckStatus($status,$STATUS_IMU2_NTBUS) ){ ##BUG: was $STATUS_NTBUS_INUSE before!!!
      ...
      if( CheckStatus($status2,$STATUS2_IMU2_8000) ){ $sa .= ' (ES)'; }
    }
...
my $PX4MP_STX  = HexstrToStr('A395');
my $PX4MP_FMT  = HexstrToStr('80');
my $PX4MP_GPS  = HexstrToStr('82');
my $PX4MP_VIBE = HexstrToStr('BD'); #128+61 = 189 = 0xBD, MP just looks for VIBE, not number!!!

my $PX4MP_STAT = HexstrToStr('EA');# 234 = 0cEA
my $PX4MP_AHRS = HexstrToStr('EB');
my $PX4MP_ANG  = HexstrToStr('EC');
my $PX4MP_CNTL = HexstrToStr('ED');
my $PX4MP_RCIN = HexstrToStr('EF');
my $PX4MP_RC2  = HexstrToStr('F0');

my $PX4MP_PERF = HexstrToStr('F1');
my $PX4MP_RAW1 = HexstrToStr('F2');
my $PX4MP_RAW2 = HexstrToStr('F3');
my $PX4MP_IMU1 = HexstrToStr('F4');
my $PX4MP_IMU2 = HexstrToStr('F5');
#my $DATA_PX4MP_ANG = HexstrToStr('EB');
my $PX4MP_PID  = HexstrToStr('F7');
my $PX4MP_MOT  = HexstrToStr('F8');

sub PX4FMT{
  my $id = shift; my $len = shift; my $cid = shift; my $format = shift; my $list = shift;
  return $PX4MP_STX.$PX4MP_FMT
       . $id . pack('C',3 + $len )
       . TrimStrWithCharToLength($cid,4,"\0").TrimStrWithCharToLength($format,16,"\0").
         TrimStrWithCharToLength($list,64,"\0");
}

#https://github.com/diydrones/ardupilot/blob/master/libraries/DataFlash/DataFlash.h
...
#GPS time: http://diydrones.com/forum/topics/onboard-log-gps-time-format

sub PX4MP_FMT_FMT{
  return PX4FMT( $PX4MP_FMT, 0x59, 'FMT', 'BBnNZ', 'Type,Length,Name,Format,Columns', );
}

sub PX4MP_FMT_GPS{
  return PX4FMT( $PX4MP_GPS, 0x2D, 'GPS',
                'BIHBcLLeeEefI', 'Status,TimeMS,Week,NSats,HDop,Lat,Lng,RelAlt,Alt,Spd,GCrs,VZ,T' );
}
sub PX4MP_GPS{
  return $PX4MP_STX.$PX4MP_GPS
          .pack( 'CLSCsllllLlfL', shift, shift, shift, shift, shift, shift, shift, shift, shift, shift, shift, shift, shift ); #S = uint16_t
}

sub PX4MP_FMT_STAT{
  return PX4FMT( $PX4MP_STAT, 4 + (1)*4+(6)*2, 'STAT',
                'IIHHHHHC', 'TimeMS,i,millis,State,Status,Status2,BusError,Voltage' );
}
sub PX4MP_STAT{
  return $PX4MP_STX.$PX4MP_STAT . pack( 'L', shift ) #L = uint32_t
          .pack( 'LSSSSSS', shift, shift, shift, shift, shift, shift ); #S = uint16_t
}

sub PX4MP_FMT_AHRS{
  return PX4FMT( $PX4MP_AHRS, 4 + (3+3+3+1+1)*2, 'AHRS',
                'Ihhhhhhccccc', 'TimeMS,GyroX,GyroY,GyroZ,AccX,AccY,AccZ,Rx,Ry,Rz,AccAbs,AccConf' );
}
sub PX4MP_AHRS{
  return $PX4MP_STX.$PX4MP_AHRS . pack( 'L', shift ) #L = uint32_t
          .pack( 'sssssssssss', shift, shift, shift, shift, shift, shift, shift, shift, shift, shift, shift ); #s = int16_t
}

sub PX4MP_FMT_ANG{
  return PX4FMT( $PX4MP_ANG, 4 + (6)*2, 'ANG',
                'Icccccc', 'TimeMS,Pitch,Roll,Yaw,Pitch2,Roll2,Yaw2' );
}
sub PX4MP_ANG{
  return $PX4MP_STX.$PX4MP_ANG . pack( 'L', shift ) #L = uint32_t
          .pack( 'ssssss', shift, shift, shift, shift, shift, shift ); #s = int16_t
}

sub PX4MP_FMT_CNTL{
  return PX4FMT( $PX4MP_CNTL, 4 + (3)*2, 'CNTL',
                'Iccc', 'TimeMS,PCntrl,RCntrl,YCntrl' );
}
sub PX4MP_CNTL{
  return $PX4MP_STX.$PX4MP_CNTL . pack( 'L', shift ) #L = uint32_t
          .pack( 'sss', shift, shift, shift ); #s = int16_t
}

sub PX4MP_FMT_RCIN{
  return PX4FMT( $PX4MP_RCIN, 4 + (3)*2, 'RCIN',
                'Ihhh', 'TimeMS,RcInPitch,RcInRoll,RcInYaw' );
}
sub PX4MP_RCIN{
  return $PX4MP_STX.$PX4MP_RCIN . pack( 'L', shift ) #L = uint32_t
          .pack( 'sss', shift, shift, shift ); #s = int16_t
}

sub PX4MP_FMT_RC2{
  return PX4FMT( $PX4MP_RC2, 4 + (8)*2, 'RC2',
                'Ihhhhhhhh', 'TimeMS,PanMode,StandBy,IRCamera,ReCenter,Script,Scr2,Scr3,Scr4' );
}
sub PX4MP_RC2{
  return $PX4MP_STX.$PX4MP_RC2 . pack( 'L', shift ) #L = uint32_t
          .pack( 'ssssssss', shift, shift, shift, shift, shift, shift, shift, shift ); #s = int16_t
}

sub PX4MP_FMT_VIBE{
  return PX4FMT( $PX4MP_VIBE, 4 + (3+3+3)*4, 'VIBE',
                'IfffIIIfff', 'TimeMS,VibeX,VibeY,VibeZ,Clip0,Clip1,Clip2,ax,ay,az' );
}
sub PX4MP_VIBE{
  return $PX4MP_STX.$PX4MP_VIBE . pack( 'L', shift )
          .pack( 'fffLLLfff', shift, shift, shift, shift, shift, shift, shift, shift, shift ); #f= float, L = uint32_t
}
...
sub DataDisplayFormatDataLine{
  ...
  my $s = '';
  if( $delim eq 'px4' ){
    $s .= PX4MP_STAT( $Data_RecordingTotalTime,
                      $DataMatrix[$DataIndex_i][$x],
                      $DataMatrix[$DataMillis_i][$x],
                      $DataMatrix[$DataState_i][$x],
                      $DataMatrix[$DataStatus_i][$x],
                      $DataMatrix[$DataStatus2_i][$x],
                      $DataMatrix[$DataI2cError_i][$x],
                      int( ($DataMatrix[$DataVoltage_i][$x]+5)/10 ) );
    $s .= PX4MP_AHRS( $Data_RecordingTotalTime,
                      $DataMatrix[$DataGx_i+0][$x], $DataMatrix[$DataGx_i+1][$x], $DataMatrix[$DataGx_i+2][$x],
                      $DataMatrix[$DataAx_i+0][$x], $DataMatrix[$DataAx_i+1][$x], $DataMatrix[$DataAx_i+2][$x],
                      $DataMatrix[$DataRx_i+0][$x], $DataMatrix[$DataRx_i+1][$x], $DataMatrix[$DataRx_i+2][$x],
                      $DataMatrix[$DataAabs_i][$x],
                      $DataMatrix[$DataAccConfidence_i][$x] );
    $s .= PX4MP_ANG(  $Data_RecordingTotalTime,
                      $DataMatrix[$DataPitch_i+0][$x], $DataMatrix[$DataPitch_i+1][$x], $DataMatrix[$DataPitch_i+2][$x],
                      $DataMatrix[$DataPitch2_i+0][$x], $DataMatrix[$DataPitch2_i+1][$x], $DataMatrix[$DataPitch2_i+2][$x] );
    $s .= PX4MP_CNTL( $Data_RecordingTotalTime,
                      $DataMatrix[$DataPitchCntrl_i+0][$x], $DataMatrix[$DataPitchCntrl_i+1][$x], $DataMatrix[$DataPitchCntrl_i+2][$x] );
    $s .= PX4MP_RCIN( $Data_RecordingTotalTime,
                      $DataMatrix[$DataPitchRcIn_i+0][$x], $DataMatrix[$DataPitchRcIn_i+1][$x], $DataMatrix[$DataPitchRcIn_i+2][$x] );
    my $packedfunctionvalues = UIntToBitstr($DataMatrix[$DataFunctionsIn_i][$x]);
    $s .= PX4MP_RC2(  $Data_RecordingTotalTime,
                      int($FunctionsInValue{substr($packedfunctionvalues,0,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,2,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,4,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,6,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,8,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,10,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,12,2)}),
                      int($FunctionsInValue{substr($packedfunctionvalues,14,2)}) );
    $s .= PX4MP_VIBE( $Data_RecordingTotalTime,
                      $vibe[0], $vibe[1], $vibe[2], 0,0,0, $acc[0], $acc[1], $acc[2] );
  }else{
...
sub dd_Save_Click{
  my $file= Win32::GUI::GetSaveFileName( -owner=> $w_DataDisplay, #$w_Main,
    ...
    -filter=> ['*.dat'=>'*.dat','*.txt'=>'*.txt','*.csv'=>'*.csv','PX4 .bin'=>'*.bin','All files'=>'*.*'],
    -pathmustexist=> 1,
  ...
  if( $file ){
    ...
    my $delim = "\t";
    if( $file =~ /\.csv$/i ){ $delim = ','; }
    if( $file =~ /\.bin$/i ){ $delim = 'px4'; binmode(F); }
...
sub dd_LiveRecording_Click{
  ...
  my $file= Win32::GUI::GetSaveFileName( -owner=> $w_DataDisplay, #$w_Main,
    ...
    if( $file ){
      ...
      if( $file =~ /\.bin$/i ){ $DataDisplayLiveRecordingDelimiter = 'px4'; binmode(F); }
...
##  if( not ConnectionIsValid() ){ Acc16PCalibrationHalt(); return 1; }
  ...
  #read data frame
#measure time
my $start = $p_Serial->get_tick_count();

  my $s= ExecuteCmd( 'Cd', $CMD_Cd_PARAMETER_ZAHL*2 );
  if( substr($s,length($s)-1,1) ne 'o' ){ TextOut("FUCK '".substr($s,length($s)-1,1)."'\r\n"); return 1; }

my $end = $p_Serial->get_tick_count();
my $duration = $end-$start;
my $last = $end - $lastupdate; $lastupdate = $end;
#TextOut( $duration." , ".$start." , ".$last."\r\n" );
...
#if( $last> 200 ){
#TextOut( $blockedcounter.": ".$duration." , ".$start." , ".$last." , ".$CdData[$CdDataAx_p]." , ".$CdData[$CdDataAy_p]." , ".$CdData[$CdDataAz_p]."\r\n" );
#$blockedcounter++;
#TextOut( $CdDataPos."\r\n" );
#}
...
$IniFile->newval( 'NT', 'NTDataLoggerTool', $NTDataLoggerToolFile );
