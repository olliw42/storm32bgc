...
my $VersionStr= '10. Oct. 2014 v0.51';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '51' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()
...
my @FirmwareVersionList= ( 'v0.51','v0.46','v0.43e','v0.41e','v0.33', ); #versions available in the flash tab selector
...
my $CMD_g_PARAMETER_ZAHL= 111; #102;
...
#  pos=>[2,5],
  ...
  name => 'Script',
  type => 'OPTTYPE_SCRIPT', len => 0, ppos => 0, min => 0, max => 0, default => 0, steps => 0,
  size => $SCRIPTSIZE,
  adr => 110,
  expert=> 4,
...
AddNewTool( ">Get Current Pitch and Roll Motor Positions", 't_GetCurrentMotorPositions' );
AddNewTool( ">Align Yaw Axis Tool...", 't_AlignYawAxis' );
AddNewTool( ">-", 0 );
AddNewTool( ">Erase EEPROM to 0xFF", 't_EraseEeprom' );
AddNewTool( ">-", 0 );
AddNewTool( ">Edit Board Name Tool...", 't_EditBoardName' );
#AddNewTool( ">-", 0 );
AddNewTool( ">Bluetooth Module Configure Tool...", 't_BTConfigTool' );
AddNewTool( ">-", 0 );
...
###SCRIPT
      $DummyOption->{textfield_script}= undef;
  return $DummyOption;
}
...
sub SetScriptTextField{
  my $text= shift;
  if( not defined $NameToOptionHash{ 'Script' } ){ return; }
  if( $text =~ /off/ ){ #modes enabled: none  disabled: #1,#2
    $NameToOptionHash{ 'Script' }->{textfield}->Disable();
  }else{ #modes enabled:
    $NameToOptionHash{ 'Script' }->{textfield}->Enable();
  }
}
...
sub onScrollSetTextfield{
  ...
  if( $Option->{name} eq 'Script Control' ){ SetScriptTextField($Option->{textfield}->Text()); }
}
...
###SCRIPT
    case 'OPTTYPE_SCRIPT' {
      ##$Option->{textfield}->Text( "!" );  ###XXX convert
      ##$Option->{textfield}->Text( Script_ConvertToText(HexstrToStr('FF')) );
      ##$SCRIPT_hexcode_buf = $value; #this is to keep also the "raw" code, in hexstr format
      $Option->{textfield}->Text( Script_ConvertToText($value) );
    }
  }
}
...
###SCRIPT
    case 'OPTTYPE_SCRIPT' {
      #$value= $Option->{textfield}->Text();
      ## $value=  $SCRIPT_hexcode_buf;
      $value = Script_ConvertToScriptCode( $Option->{textfield}->Text() );
    }
  }
  return $value;
}
...
###SCRIPT
      case ['OPTTYPE_SCRIPT'] {
        if( not defined $DummyOption->{textfield_script} ){
          #create textfield
          $textfield= Script_CreateTextField();
#          $textfield->Hide();
          $DummyOption->{textfield_script}= $textfield;
        }
        $textfield= $DummyOption->{textfield_script};
        $textfield->Show();
        $setfield= undef;
      }
    }
...
##flash using SystemBootloader
    ...
    my $portnr= ExtractCom( $f_Tab{flash}->m_flash_STM32ProgrammerComPort->Text() );  ####YYYYYYYY COM flash
    if( $portnr eq '' ){
...
#	           SC_RESTORE, SC_WAIT, SC_SET, SC_SETMINMAX,
#	           SC_STOPFF = 0xFF,
#	           SCRIPTCMDUNDEFINED } SCRIPTCMDTYPE;


my @SCRIPT_CMDS= ( 'STOP', 'RESTART', 'REPEAT', 'CASE#DEFAULT', 'CASE#1', 'CASE#2', 'CASE#3', 'JUMP',
                   'RESTORE', 'WAIT', 'SET', 'SETMINMAX', '' );


my $SCRIPT_hexcode_buf = ''; #this is to store the original "raw" script code, in hexstr formatr


sub Script_CreateTextField{
  my $i = 2;
  my $j = 1;
  my $xpos= 20+($i)*$OPTIONSWIDTH_X;
  my $ypos= 10 + ($j)*$OPTIONSWIDTH_Y;
  my %textfieldparams=( -name=> 'OptionField_script', -font=> $StdWinFont,
    -pos=> [$xpos+2,$ypos+13], -size=> [315,155+3], #-size=> [300,200], #-size=> [300,245],
    -vscroll=> 1, -multiline=> 1, -readonly => 1,
    -background => $OptionInvalidColor,
  );
  return $f_Tab{functions}->AddTextfield(%textfieldparams);
  #my $textfield= $f_Tab{functions}->AddTextfield(%textfieldparams);
  #$textfield->Hide();
  #$DummyOption->{textfield_script}= $textfield;
}
...
$f_Tab{functions}->AddButton( -name=> 'm_EditScript', -font=> $StdWinFont,
  -text=> 'Edit Script', -pos=> [$xpos+2*$OPTIONSWIDTH_X,$ypos+4+5*$OPTIONSWIDTH_Y], -width=> 120,
  -onClick=> sub{ ScriptEditorShow(); 1;},
);


sub Script_ConvertToScriptCode{
  return $SCRIPT_hexcode_buf;
}
...
sub Script_ConvertToText{ #script is in hexstr format
  my $script_hex = shift;
  my $script_showpc_flag= shift;
  if( not defined $script_showpc_flag ){ $script_showpc_flag=''; }
  $SCRIPT_hexcode_buf = $script_hex;
  if( length($script_hex)<2 ){ $SCRIPT_hexcode_buf = ''; return ''; }#'??'; }
#TextOut( "!".$script_hex."!" );
  my $script_text= ''; my $pc=0; my $indent= '';

  for( $pc=0; $pc<length($script_hex); $pc+=2 ){
    if($script_showpc_flag ne 'nopc'){ $script_text.= sprintf( "%2d", $pc/2).':  '; }
    ...
    if( $op eq 'FF' ){
      ...
      if(( $s eq 'CASE#DEFAULT' )or( $s eq 'CASE#1' )or( $s eq 'CASE#2' )or( $s eq 'CASE#3' )){
        $script_text.= $s;
...
my $w_Script_Menubar= Win32::GUI::Menu-> new(
  'Script' => '',
    '>New', 'script_New',
    '>Load from File...', 'script_Load',  #gets Filename, reads file, and extracts all settings
    '>Save to File...', 'script_Save',  #saves all settings into a .ini file
    '>-', 0,
    '>Compile', 'script_Compile',  #saves all settings into a .ini file
    '>-', 0,
...
sub script_Compile_Click{ Script_CompileText(); 0; }

sub script_Exit_Click{
  my($hexcode,$stateness,$error) = Script_CompileText();
  if(( $error eq '' )&&( defined $NameToOptionHash{'Script'} )){
    while( length($hexcode)<2*$SCRIPTSIZE ){ $hexcode.='FF'; }
    SetOptionField( $NameToOptionHash{'Script'}, $hexcode );
    $NameToOptionHash{'Script'}->{textfield}->Change( -background => $OptionModifiedColor );
  }
  $w_Script->Hide();
  0;
}
...
sub ScriptEditorShow{
  ...
  #$w_Script->script_Text->Text( Script_ConvertToScriptCode($SCRIPT_hexcode_buf) );
  $w_Script->script_Text->Text( Script_ConvertToText($SCRIPT_hexcode_buf,'nopc') );
  $w_Script->Show();
}

$w_Script-> AddTextfield( -name=> 'script_Text', -font=> $ScriptEditorTextFont, #-font=> $StdWinFont,
  -pos=> [-1,-1], -size=> [$xsize-4,$ysize-42], #-size=> [$xsize-14,$ysize-52],
  -hscroll=> 1, -vscroll=> 1,
...
sub Script_CompileText{
  my $code= $w_Script->script_Text->Text();
  $code = uc( $code ).' '; #add a white char to simplify regex
#TextOut( "!".$code."!\r\n" );
  my $token;
  my $hexcode=''; my $stateness= '';
...
#TextOut( $hexcode."\r\n\r\n" );
  ...
  # every case muts occur only once, if any occurs it must start with CASE#1, and no holes must occur
  if( $casepclist[3]>0 ){
  ...
  if( $casepclist[3]>0 ){ $stateness = 'FOURSTATE'; }
  elsif( $casepclist[2]>0 ){ $stateness = 'THREESTATE'; }
  elsif( $casepclist[1]>0 ){ $stateness = 'TWOSTATE'; }
  else{ $stateness = 'CONTINUOUS'; }

#  TextOut( "\r\n".'Compile... DONE!'."\r\n".$stateness.','.$hexcode."\r\n" );
  TextOut( "\r\n".'Compile... DONE!'."\r\n".$hexcode."\r\n" );
ERROR:
  if( $error ne '' ){ TextOut( "\r\n".'Compile... ERROR!'."\r\n".$error."\r\n" ); }
  return ($hexcode,$stateness,$error);
}
...
sub editboardname_Window_Terminate{ $w_EditBoardName->Hide(); 0; }

$xpos= 20;
$ypos= 20;
$w_EditBoardName->AddLabel( -name=> 'editboardname_Text1_label', -font=> $StdWinFont,
  -text=> "With this tool you can change the name of the STorM32-BGC board.",
  -pos=> [$xpos,$ypos], -width=> 420,  -height=>30,
  -background=>$EditBoardNameBackgroundColor, -foreground=> [255,255,255],
);
$ypos+= 35;
$w_EditBoardName->AddLabel( -name=> 'editboardname_Text1b_label', -font=> $StdWinFont,
  -text=> "Name",
  -pos=> [$xpos+64,$ypos], -multiline=>1, -height=>3*13+10, -width=> 420,
  -background=>$EditBoardNameBackgroundColor, -foreground=> [255,255,255],
);
...
$xpos= 20;
$ypos+= 35 ;
$w_EditBoardName->AddLabel( -name=> 'editboardname_Text4_label', -font=> $StdWinFont,
  -text=> "When done press OK; or press Cancel to abort.
NOTE: To keep the change, do a Write+Store after closing the tool!",
...
#  if( $DataDisplay_IsRunning ){ TextOut("\r\nStop DataDisplay before calling AlignYawAxisTool!\r\n"); }
  ...
  $w_EditBoardName->editboardname_Name->Text( $NameToOptionHash{'Name'}->{textfield}->Text() );
...
sub editboardname_OK_Click{
  ...
  while( length($name)<16 ){ $name.=' '; }
  ExecuteCommandWritewoGet( 'xn', 'Edit Board Name Tool', '', $name );
...
#TextOut( $AlignYawRes2."!" );TextOut( substr($AlignYawRes2,12,8)."!!" );
  ...
  if( $AlignYawRes1 eq 't' or $AlignYawRes1 eq 'c' or
      ...
      $AlignYawRes3 eq 't' or $AlignYawRes3 eq 'c'     ){
    TextOut( "\r\n".'Align Yaw Axis Tool... ABORTED!'."\r\n" );
    $w_AlignYaw->Hide();
    return;
  }
...
sub AlignYawUndowoOpenClose{
  TextOut( 'Undo... ' ); #do the commands in reverese order
  #undo: set Rc Yaw Offset
  SendMavlinkCmd2( '04'. substr($AlignYawRes3,12,8) );
  _delay_ms( 1000 ); #wait to give command a change before it is switched off
...
#  ExecuteCommandFullwoGet( 'xm', 'Get current motor directions', '' );
  if( not OpenPort() ){ TextOut("\r\n"."Port XXX FXßpd!"."\r\n"); return; }
  TextOut_( 'xy'.'... ' );
  my $s= ExecuteCmd( 'xy' ); #  my $s= ExecuteCmd( 'Cd', $CMD_Cd_PARAMETER_ZAHL*2 );
  AlignYawUndowoOpenClose();
