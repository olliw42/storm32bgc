...
my $VersionStr= '28. Mar. 2015 v0.68';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '66' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()
...
my @STorM32BGCBoardList= (
{
  ...
  name => 'v1.31 with F103RC',
  uc => 'F103RC',
  hexfile => 'storm32bgc_v130_f103rc',
}
);

my @FirmwareVersionList= ( 'v0.68','v0.67e','v0.56' ); #versions available in the flash tab selector
...
my $FunctionInputMax= 39 -1; #34; #32;

my $SCRIPTSIZE= 128;
my $CMD_g_PARAMETER_ZAHL= 124; #number of values transmitted with a 'g' get data command
...
##--- RC INPUTS tab --------------------
  ...
  name => 'Rc Hysteresis',
  type => 'OPTTYPE_UI', len => 0, ppos => 0, min => 0, max => 20, default => 5, steps => 1,
  size => 2,
  adr => 105,
  unit => 'us',
  expert=> 2,
  ...
  size => 1,
  adr => 59,
  choices => [ 'absolute', 'relative', 'relative turn around', ], #'relative slip ring' ],
  expert=> 2,
},{
  name => 'Rc Yaw Min',
...
###  choices => [ 'off', 'gimbal support', 'gimbal support xy', 'yaw bracket', 'yaw bracket xy' ],
  choices => [ 'off', 'full', 'full xy', 'light', 'light xy' ],
  expert=> 4,
  ...
  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 8, default => 0, steps => 1,
  size => 1,
  adr => 41,
  choices => [ 'off',  'sum ppm 6', 'sum ppm 7', 'sum ppm 8', 'sum ppm 10', 'sum ppm 12',
               'spektrum 10 bit', 'spektrum 11 bit', 'sbus' ],
  expert=> 4,
  column=> 2,
...
##--- EXPERT tab  --------------------
  ...
  name => 'Motor Mapping',#'Motor Configuration',
  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 1, default => 0, steps => 1,
  size => 1,
  adr => 40,
...
my @SetupTabList= ( 'main', 'pan', 'rcinputs', 'functions', 'gimbalsetup', 'expert', 'gimbalconfig', 'scripts', ); #this is the name of the experts tabs
...
my $m_Menubar= Win32::GUI::Menu-> new(
  ...
  'Tools' => '',
    ...
    ">-", 0,
    ">Change Uart Baudrate Tool... (experts only)", 't_ChangeBaudrate',
    ">Mavlink RC Tool... (experts only)", 't_MavlinkTool',
    @ExpertTool,
    ">-", 0,
...
$f_Tab{main}= $w_Main->w_Tab->InsertItem(-text=> '      Main');
...
sub t_ExpertTool_Click{
  ...
  $f_Tab{expert}->Move($x+$ColNumber*$OPTIONSWIDTH_X+40-15+20-385-180+1,$y+50-12);
  $f_Tab{expert}->Show(); 0;
}
...
$f_Tab{main}->AddLabel( -name=> 'UseSimplifiedPID_label', -font=> $StdWinFont,
  -text=> 'Use simplified PID tuning', -pos=> [$xpos,$ypos],
);
$f_Tab{main}->AddCheckbox( -name  => 'UseSimplifiedPID', -font=> $StdWinFont,
  -pos=> [$xpos+130-10,$ypos+1], -size=> [12,12],
);
$f_Tab{main}->UseSimplifiedPID->Checked(0);

$xpos= 20+(1)*$OPTIONSWIDTH_X;
$ypos= 10 + ($SimplifiedPIDY+0)*$OPTIONSWIDTH_Y -1;
$f_Tab{main}->AddLabel( -name=> 'UseSimplifiedPID_frame', -font=> $StdWinFont,
  -pos=> [$xpos-5,$ypos-5], -size=> [505+10,127+10], -background => $SimplifiedPIDBackgroundColor,
);
$f_Tab{main}->UseSimplifiedPID_frame->Hide();

$xpos= 20 + (1)*$OPTIONSWIDTH_X;
$ypos= 10+($SimplifiedPIDY+0)*$OPTIONSWIDTH_Y;
my $PitchDamping_label= $f_Tab{main}->AddLabel( -name=> 'PitchDamping_label', -font=> $StdWinFont,
  -text=> 'Pitch Damping', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $PitchDamping_value= $f_Tab{main}->AddLabel( -name=> 'PitchDamping_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2 -1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $PitchDamping_slider= $f_Tab{main}->AddTrackbar( -name=> 'PitchDamping_sliderfield', -font=> $StdWinFont,
...
my $PitchStability_label= $f_Tab{main}->AddLabel( -name=> 'PitchStability_label', -font=> $StdWinFont,
  -text=> 'Pitch Stability', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $PitchStability_value= $f_Tab{main}->AddLabel( -name=> 'PitchStability_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2-1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $PitchStability_slider= $f_Tab{main}->AddTrackbar( -name=> 'PitchStability_sliderfield', -font=> $StdWinFont,
  -pos=> [$xpos,$ypos+13], -size=> [150,23], -background => $SimplifiedPIDBackgroundColor,
  -aligntop => 1, -autoticks => 0,
  -onScroll => sub{ onScrollSetPID2field( 'Pitch' ); 1; },
);
...
$ypos= 10+($SimplifiedPIDY+0)*$OPTIONSWIDTH_Y;
my $RollDamping_label= $f_Tab{main}->AddLabel( -name=> 'RollDamping_label', -font=> $StdWinFont,
  -text=> 'Roll Damping', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $RollDamping_value= $f_Tab{main}->AddLabel( -name=> 'RollDamping_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2 -1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $RollDamping_slider= $f_Tab{main}->AddTrackbar( -name=> 'RollDamping_sliderfield', -font=> $StdWinFont,
  -pos=> [$xpos,$ypos+13], -size=> [150,23], -background => $SimplifiedPIDBackgroundColor,
  -aligntop => 1, -autoticks => 0,
  -onScroll => sub{ onScrollSetPID2field( 'Roll' ); 1; },
);
$ypos= 10+($SimplifiedPIDY+1)*$OPTIONSWIDTH_Y;
my $RollStability_label= $f_Tab{main}->AddLabel( -name=> 'RollStability_label', -font=> $StdWinFont,
  -text=> 'Roll Stability', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $RollStability_value= $f_Tab{main}->AddLabel( -name=> 'RollStability_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2-1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $RollStability_slider= $f_Tab{main}->AddTrackbar( -name=> 'RollStability_sliderfield', -font=> $StdWinFont,
  -pos=> [$xpos,$ypos+13], -size=> [150,23], -background => $SimplifiedPIDBackgroundColor,
  -aligntop => 1, -autoticks => 0,
  -onScroll => sub{ onScrollSetPID2field( 'Roll' ); 1; },
);
...
$ypos= 10+($SimplifiedPIDY+0)*$OPTIONSWIDTH_Y;
my $YawDamping_label= $f_Tab{main}->AddLabel( -name=> 'YawDamping_label', -font=> $StdWinFont,
  -text=> 'Yaw Damping', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $YawDamping_value= $f_Tab{main}->AddLabel( -name=> 'YawDamping_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2 -1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $YawDamping_slider= $f_Tab{main}->AddTrackbar( -name=> 'YawDamping_sliderfield', -font=> $StdWinFont,
  -pos=> [$xpos,$ypos+13], -size=> [150,23], -background => $SimplifiedPIDBackgroundColor,
  -aligntop => 1, -autoticks => 0,
  -onScroll => sub{ onScrollSetPID2field( 'Yaw' ); 1; },
);
$ypos= 10+($SimplifiedPIDY+1)*$OPTIONSWIDTH_Y;
my $YawStability_label= $f_Tab{main}->AddLabel( -name=> 'YawStability_label', -font=> $StdWinFont,
  -text=> 'Yaw Stability', -pos=> [$xpos,$ypos-1], -size=> [150,13+2], -background => $SimplifiedPIDBackgroundColor,
);
my $YawStability_value= $f_Tab{main}->AddLabel( -name=> 'YawStability_value', -font=> $StdWinFont,
  -text=> '0', -pos=> [$xpos+94 +15,$ypos-1], -size=> [30,13+2-1], #-background => $SimplifiedPIDBackgroundColor,
  -align => 'center',
);
my $YawStability_slider= $f_Tab{main}->AddTrackbar( -name=> 'YawStability_sliderfield', -font=> $StdWinFont,
  -pos=> [$xpos,$ypos+13], -size=> [150,23], -background => $SimplifiedPIDBackgroundColor,
  -aligntop => 1, -autoticks => 0,
  -onScroll => sub{ onScrollSetPID2field( 'Yaw' ); 1; },
);
...
sub UseSimplifiedPID_Click{
  if( $f_Tab{main}->UseSimplifiedPID->Checked() ){
    $f_Tab{main}->UseSimplifiedPID_frame->Show();
...
my $PitchIMCf0_label= $f_Tab{main}->AddLabel( -name=> 'PitchIMUf0_label', -font=> $StdWinFont,
  -text=> '  f0 =', -pos=> [$xpos,$ypos], -size=> [150,13+2],
);
my $PitchIMCd_label= $f_Tab{main}->AddLabel( -name=> 'PitchIMUd_label', -font=> $StdWinFont,
  -text=> '   d =', -pos=> [$xpos,$ypos+16], -size=> [150,13+2],
);
my $PitchIMCtau_label= $f_Tab{main}->AddLabel( -name=> 'PitchIMUtau_label', -font=> $StdWinFont,
  -text=> 'tau =', -pos=> [$xpos,$ypos+32], -size=> [150,13+2],
);

$xpos= 30+(2)*$OPTIONSWIDTH_X;
$ypos= 10 + (6)*$OPTIONSWIDTH_Y;
my $RollIMCf0_label= $f_Tab{main}->AddLabel( -name=> 'RollIMUf0_label', -font=> $StdWinFont,
  -text=> '  f0 =', -pos=> [$xpos,$ypos], -size=> [150,13+2],
);
my $RollIMCd_label= $f_Tab{main}->AddLabel( -name=> 'RollIMUd_label', -font=> $StdWinFont,
  -text=> '   d =', -pos=> [$xpos,$ypos+16], -size=> [150,13+2],
);
my $RollIMCtau_label= $f_Tab{main}->AddLabel( -name=> 'RollIMUtau_label', -font=> $StdWinFont,
  -text=> 'tau =', -pos=> [$xpos,$ypos+32], -size=> [150,13+2],
);

$xpos= 30+(3)*$OPTIONSWIDTH_X;
$ypos= 10 + (6)*$OPTIONSWIDTH_Y;
my $YawIMCf0_label= $f_Tab{main}->AddLabel( -name=> 'YawIMUf0_label', -font=> $StdWinFont,
  -text=> '  f0 =', -pos=> [$xpos,$ypos], -size=> [150,13+2],
);
my $YawIMCd_label= $f_Tab{main}->AddLabel( -name=> 'YawIMUd_label', -font=> $StdWinFont,
  -text=> '   d =', -pos=> [$xpos,$ypos+16], -size=> [150,13+2],
);
my $YawIMCtau_label= $f_Tab{main}->AddLabel( -name=> 'YawIMUtau_label', -font=> $StdWinFont,
  -text=> 'tau =', -pos=> [$xpos,$ypos+32], -size=> [150,13+2],
);

my $ShowIMCCalculator= 1;
$f_Tab{main}->AddButton( -name=> 'ShowIMCCalculator',
...
sub ShowIMCCalculator_Click{
  ...
  if( $ShowIMCCalculator==0 ){ #unfold
    $ShowIMCCalculator= 1;
    $f_Tab{main}->ShowIMCCalculator->Text( "-");
    $f_Tab{main}->PitchIMUf0_label->Show();
    $f_Tab{main}->PitchIMUd_label->Show();
    $f_Tab{main}->PitchIMUtau_label->Show();
    $f_Tab{main}->RollIMUf0_label->Show();
    $f_Tab{main}->RollIMUd_label->Show();
    $f_Tab{main}->RollIMUtau_label->Show();
    $f_Tab{main}->YawIMUf0_label->Show();
    $f_Tab{main}->YawIMUd_label->Show();
    $f_Tab{main}->YawIMUtau_label->Show();
  }else{ #fold
    $ShowIMCCalculator= 0;
    $f_Tab{main}->ShowIMCCalculator->Text( "+");
    $f_Tab{main}->PitchIMUf0_label->Hide();
    $f_Tab{main}->PitchIMUd_label->Hide();
    $f_Tab{main}->PitchIMUtau_label->Hide();
    $f_Tab{main}->RollIMUf0_label->Hide();
    $f_Tab{main}->RollIMUd_label->Hide();
    $f_Tab{main}->RollIMUtau_label->Hide();
    $f_Tab{main}->YawIMUf0_label->Hide();
    $f_Tab{main}->YawIMUd_label->Hide();
    $f_Tab{main}->YawIMUtau_label->Hide();
  }
}
...
sub ExecuteReadCalibrationData{
  ...
  my ($ret,$s)= ExecuteCommandFullwoGet( 'Cr', 'Read calibration data', '', 18*2, 1 );
  if( $ret==0 ){ return 0; }
...
sub PopulateOptions{
  ...
  foreach my $Option (@OptionList){
    ...
    $label= $f_Tab{$tab_ptr}->AddLabel(-name=> 'OptionField'.$nr.'_label', -font=> $StdWinFont,
      -text=> $Option->{name}, -pos=> [$xpos,$ypos-1], -size=> [145,15],
    );
...
sub onScrollSetTextfield{
  ...
  if( $Option->{name} =~ /Pitch [PID]$/ ){ SetIMCCalculator('Pitch'); SetPID2('Pitch'); }
  elsif( $Option->{name} =~ /Roll [PID]$/ ){ SetIMCCalculator('Roll');  SetPID2('Roll'); }
  elsif( $Option->{name} =~ /Yaw [PID]$/ ){ SetIMCCalculator('Yaw');  SetPID2('Yaw'); }
...
my $ShareSettingsYsize= 870-32; # -50;

my $w_ShareSettings= Win32::GUI::DialogBox->new( -name=> 'sharesettings_Window', -parent => $w_Main, #-font=> $StdWinFont,
...
sub ShareSettingsShow{
  ...
  #my ($len,$h)= $w_Main->GetTextExtentPoint32( $text1_name );
...
sub ConvertStr2StatusBarStr{
  ...
  if( CheckStatus($s2,$STATUS_IMU_PRESENT) ){
    if( CheckStatus($s2,$STATUS_IMU_HIGHADR) ){ $result .= ' @ HIGH ADR'; }else{ $result .= ' @ LOW ADR'; }
    if( CheckStatus($s3,$STATUS2_IMU_8000) ){ $result .= ' (ES)'; }
  }else{ $result .= ' none'; }
  $result .= '   IMU2:';
  if( CheckStatus($s2,$STATUS_IMU2_PRESENT) ){
    if( CheckStatus($s2,$STATUS_IMU2_HIGHADR) ){
      $result .= ' @ HIGH ADR = on-board IMU'; }else{ $result .= ' @ LOW ADR = external IMU'; }
    if( CheckStatus($s3,$STATUS2_IMU2_8000) ){ $result .= ' (ES)'; }
  }else{ $result .= ' none'; }
  $result .= '    MAG:';
  if( CheckStatus($s2,$STATUS_MAG_PRESENT) ){ $result .= '  avail.'; }else{ $result .= ' none'; }
  $result .= '    BAT:';
  if( CheckStatus($s2,$STATUS_BAT_ISCONNECTED) ){ $result .= ' CONNECTED'; }else{ $result .= ' none'; }
  $result .= '    VOLTAGE:';
  if( CheckStatus($s2,$STATUS_BAT_VOLTAGEISLOW) ){ $result .= ' LOW'; }else{ $result .= ' SUFFICIENT'; }
  $result .= ' '.sprintf("%.2f", $data[4]/1000.0).' V';
  $result .= '    STATE:';
  my ($sss)= GetState( $data[0] ); #state
  if( $sss ne '' ){ $result .= ' '.$sss; }else{ $result .= ' unknown'; }
  return $result;
}
...
#sets if details should be displayed in ExecuteGetCommand2()
my $ReadDetailsOut = 1;
sub SetReadDetailsOut{ $ReadDetailsOut = shift; }

#sets if a Delay should be waited before a Read in ExecuteCommandFullwoGet() -> calls ExecuteGetCommand2()
my $DelayBeforeGet = 0;
sub SetDelayBeforeGet{ $DelayBeforeGet = shift; }

#sets if a Read should be done in ExecuteCommandFullwoGet() -> calls ExecuteGetCommand2()
my $WithGet = 0;
...
#flags:
# $ExtendedTimoutFirst
# allows to momentarily increase the timeout for foirst char, this is needed for e.g. StoreToEERPOM
sub ReadPort{
  ...
  do{
    if(( $count==0 )and( $p_Serial->get_tick_count() > $tmofirst )){ return ''; } #timeout!
...
sub ExecuteCmd{
## this diesn't help a lot against the heartbeat
##  my $cmd = shift;
...
sub m_Timer_Timer{
  ...
  if( $DataDisplay_IsRunning ){
    ...
    if( $MainTimerCounter!=0 ){ return 1; }
    FlushPort(); #this is to get back in track in case of issue
    $s = ExecuteCmdTimer( 's', 5*2 );
  }
#Win32::GUI::DoEvents();
  ...
  if( $DataDisplay_IsRunning ){
    ...
    my $res = ConvertStr2StatusBarStr( $s );
    $w_Main->m_StatusField->Text( $res );
  }
  1;
}
...
sub ExecuteCommandFull{
  my $CMD= shift; my $MSG= shift; my $MSG2= shift; my $TEST= shift;
  if( $TEST ){
    TextOut( "\r\n".$MSG.'... Please wait!' );
  ...
  if( $WithGet ){
    SetReadDetailsOut(0);
    if( not ExecuteGetCommand2() ){ goto EXIT; }
    SetOptionsLoaded(1);
  }
  $WithGet = 0;
  TextOut( "\r\n".$MSG.'... DONE!' );
...
sub ExecuteCommandFullwoGet{
  SetWithGet(0);
  return  ExecuteCommandFull( shift, shift, shift, shift );
}
...
sub ExecuteStoreToEeprom{
  ...
  return ExecuteCommandFullwoGet( 'xs', 'Store to EEPROM', '', 1 );
}

sub ExecuteRetrieveFromEeprom{
  return ExecuteCommandFullwGet( 'xr', 'Retrieve from EEPROM', '', 1 );
}

sub ExecuteEraseEeprom{
  SetExtendedTimoutFirst(1000); #???? is this needed ???
  return ExecuteCommandFullwoGet( 'xc', 'Erase EEPROM', 'NOTE: Please reset or power up the BGC board for proper operation!', 1 );
}

sub ExecuteDefault{
  return ExecuteCommandFullwGet( 'xd', 'Set to Default', '', 1 );
}

sub ExecuteLevelGimbal{
  return ExecuteCommandFullwoGet( 'xl', 'Level gimbal', '', 1 );
}
...
sub ExecuteCalibrateRcTrim{
  ...
  my $ret = ExecuteCommandFullwGet( 'CR', 'Calibrate rc trims', '', 1 );
  $Execute_IsRunning = 0;
  return $ret;
}
...
sub ExecuteRestartController{
  ...
  SetDelayBeforeGet(1500);
  my $ret= ExecuteCommandFullwGet( 'xx', 'Restart controller', '', 1 );
#  SetOptionsLoaded(0);
  if( not $ret ){ return 0; }
  #handle case if Data Display is running
...
sub Script_GetParameterAdr{
  my $code= shift;
  $code =~ s/^\s*"([A-Z0-9# ]+?)"\s(.*)/$2/; #finds paramter name string, and removes it
...
sub ConfigureGimbalAlignYawUndo{
  ...
  SendMavlinkCmdwoOut( '04'. substr($ConfigureGimbal_RcYawMaxRes,12,8) );
  TextOut( $ConfigureGimbal_RcYawMaxAdr.',' );
  SendMavlinkCmdwoOut( '04'. substr($ConfigureGimbal_RcYawMinRes,12,8) );
  TextOut( $ConfigureGimbal_RcYawMinAdr.',' );
  SendMavlinkCmdwoOut( '04'. substr($ConfigureGimbal_RcYawOffsetRes,12,8) );
  TextOut( $ConfigureGimbal_RcYawOffsetAdr.',' );
  _delay_ms( 1000 ); #wait to give the Offset command a chance before it is switched off
  SendMavlinkCmdwoOut( '04'. substr($ConfigureGimbal_RcYawModeRes,12,8) );
  TextOut( $ConfigureGimbal_RcYawModeAdr.',' );
  SendMavlinkCmdwoOut( '04'. substr($ConfigureGimbal_RcYawRes,12,8) );
...
sub ConfigureGimbalAlignYawMove{
  ...
  TextOut( "\r\n".'Move...' );
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawOffsetAdr).'00'.$v );
  TextOut( ' ok' );
}
...
##  if( not ConnectionIsValid() ){ goto WERROR; }
  ...
  $res1= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'.'0300' );
  SetOptionField( $Option, 3 );
  $Option= $NameToOptionHash{'Roll Motor Usage'};
  $res2= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'.'0300' );
  SetOptionField( $Option, 3 );
  $Option= $NameToOptionHash{'Yaw Motor Usage'};
  $res3= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'.'0300' );
  SetOptionField( $Option, 3 );
...
if($ConfigureGimbal_StepNr==0){
  ...
  my $s2= UIntToBitstr( $data[1] ); #status
  $s = '';
  if(( $doimu1 )and( not CheckStatus($s2,$STATUS_IMU_PRESENT) )){
    $s .= 'Imu1 is not present or not healthy!' . "\r\n";
  }
  if(( $doimu2 )and( not CheckStatus($s2,$STATUS_IMU2_PRESENT) )){
...
if($ConfigureGimbal_StepNr==2){
  ...
  if( $doimu1 and( $ConfigureGimbal_Imu1No>=0 )){
    ...
    $res1= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($ConfigureGimbal_Imu1No).'00' );
  ...
  if( $doimu2 and( $ConfigureGimbal_Imu2No>=0 )){
    ...
    $res2= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($ConfigureGimbal_Imu2No).'00' );
...
if($ConfigureGimbal_StepNr==1){
  ...
  if( $poles>=12 ){
    ...
    $res1= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($poles).'00' );
...
if($ConfigureGimbal_StepNr==1){
  ...
  $res1= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '0200' );
  SetOptionField( $Option, 2 );
  $Option= $NameToOptionHash{'Roll Motor Direction'};
  $res2= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '0200' );
  SetOptionField( $Option, 2 );
  $Option= $NameToOptionHash{'Yaw Motor Direction'};
  $res3= SendMavlinkCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '0200' );
  SetOptionField( $Option, 2 );
  #check
  if(( $res1 =~ /[tc]/ )or( $res2 =~ /[tc]/ )or( $res3 =~ /[tc]/ )){ goto WERROR; }
  TextOut( ' ok' );
...
if($ConfigureGimbal_StepNr==1){
  ...
  while( $checkvoltageloop ){
    ...
    my @data = unpack( "v$CMD_d_PARAMETER_ZAHL", $s ); ####THIS IS A BUG; RIGHT?????
    my $s2= UIntToBitstr( $data[1] ); #status
    if( not CheckStatus($s2,$STATUS_IMU_PRESENT) ){ goto IMUERROR; }
    if( CheckStatus($s2,$STATUS_BAT_ISCONNECTED) and not CheckStatus($s2,$STATUS_BAT_VOLTAGEISLOW) ){
...
if($ConfigureGimbal_StepNr==1){
  ...
  $Option= $NameToOptionHash{'Pitch Motor Direction'};
  $res1= SendMavlinkCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' , 12 );
  SetOptionField( $Option, substr($res1,2*8,2) );
  $Option= $NameToOptionHash{'Roll Motor Direction'};
  $res2= SendMavlinkCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' , 12 );
  SetOptionField( $Option, substr($res2,2*8,2) );
  $Option= $NameToOptionHash{'Yaw Motor Direction'};
  $res3= SendMavlinkCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' , 12 );
  SetOptionField( $Option, substr($res3,2*8,2) );
  #finish things, and go to next page
...
if($ConfigureGimbal_StepNr==0){
  ...
  $w_ConfigureGimbal->configuregimbal_WelcomeText2->Text(
    'The current pitch and roll motor positions will read from the board and copied to the respective startup position parameters.'
    . "\r\n". "\r\n".
    'Press >Continue< to write the values to the board, and to continue.' );
  $ConfigureGimbal_StepNr= 1;
  $w_ConfigureGimbal->configuregimbal_Continue->Enable();
...
if($ConfigureGimbal_StepNr==1){
  ...
  $res1= SendMavlinkCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' , 12 );
  SetOptionField( $Option, HexstrToDez(substr($res1,2*9,2).substr($res1,2*8,2)) );
  $Option= $NameToOptionHash{'Roll Startup Motor Pos'};
  $res2= SendMavlinkCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' , 12 );
  SetOptionField( $Option, HexstrToDez(substr($res2,2*9,2).substr($res2,2*8,2)) );
  #finish things, and go to next page
...
if($ConfigureGimbal_StepNr==0){
  ...
  TextOut( $ConfigureGimbal_RcYawAdr.',' );
  $ConfigureGimbal_RcYawRes= SendMavlinkCmdwoOut( '03'. UCharToHexstr($ConfigureGimbal_RcYawAdr).'00', 8+2*2 );
  $ConfigureGimbal_RcYawModeAdr= $NameToOptionHash{'Rc Yaw Mode'}->{adr};
  TextOut( $ConfigureGimbal_RcYawModeAdr.',' );
  $ConfigureGimbal_RcYawModeRes= SendMavlinkCmdwoOut( '03'. UCharToHexstr($ConfigureGimbal_RcYawModeAdr).'00', 8+2*2 );
  $ConfigureGimbal_RcYawOffsetAdr= $NameToOptionHash{'Rc Yaw Offset'}->{adr};
  TextOut( $ConfigureGimbal_RcYawOffsetAdr.',' );
  $ConfigureGimbal_RcYawOffsetRes= SendMavlinkCmdwoOut( '03'. UCharToHexstr($ConfigureGimbal_RcYawOffsetAdr).'00', 8+2*2 );
  $ConfigureGimbal_RcYawMinAdr= $NameToOptionHash{'Rc Yaw Min'}->{adr};
  TextOut( $ConfigureGimbal_RcYawMinAdr.',' );
  $ConfigureGimbal_RcYawMinRes= SendMavlinkCmdwoOut( '03'. UCharToHexstr($ConfigureGimbal_RcYawMinAdr).'00', 8+2*2 );
  $ConfigureGimbal_RcYawMaxAdr= $NameToOptionHash{'Rc Yaw Max'}->{adr};
  TextOut( $ConfigureGimbal_RcYawMaxAdr );
  $ConfigureGimbal_RcYawMaxRes= SendMavlinkCmdwoOut( '03'. UCharToHexstr($ConfigureGimbal_RcYawMaxAdr).'00', 8+2*2 );
  #check
  ...
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawAdr).'00'.'0000' );
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawModeAdr).'00'.'0000' );
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawOffsetAdr).'00'.'0000' );
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawMinAdr).'00'.'3EFE' ); #-450 = FE3E
  SendMavlinkCmdwoOut( '04'. UCharToHexstr($ConfigureGimbal_RcYawMaxAdr).'00'.'C201' ); #+450 = 01C2
  TextOut( 'ok' );
...
my $MavlinkDetailsOut = 1;
my $MavlinkConnectionTest = 1;
...
sub SendMavlinkCmd{
  my $DetailsOut = $MavlinkDetailsOut;
  $MavlinkDetailsOut = 1;
  my $ConnectionTest = $MavlinkConnectionTest;
  $MavlinkConnectionTest = 1;
...
#TextOut( "!$msglen!" );
  my $cmd= $MAVRCSTX. $msglen .'00'.'52'.'43'. $msg . '33'.'34'; #crc check is not activated, hence dummy crc
if($MavlinkRcUse0xFE){
  $msglen = UCharToHexstr( length($msg)/2 - 1  +3);
  $cmd= $MAVSTX. $msglen .'00'.'52'.'43'. 'EA'.'47'.'43'. $msg . '33'.'34'; #crc check is not activated, hence dummy crc
  $doread += 3;
}
  if( $ConnectionTest ){
...
#  my $tmo= $p_Serial->get_tick_count() + 20*$ExecuteCmdTimeOut; #timeout in 100 ms
  ...
  my $tmo = $p_Serial->get_tick_count() + $timeout;
  do{
    if( $p_Serial->get_tick_count() > $tmo  ){ return 't'; }
    my ($i, $s) = $p_Serial->owread_overlapped(1);
    $count+= $i;
    $result.= $s;
...
sub SendMavlinkCmdwoOut{
  $MavlinkDetailsOut = 0;
  $MavlinkConnectionTest = 0;
  return SendMavlinkCmd( shift, shift );
}
...
my $MavlinkXsize= 510;
my $MavlinkYsize= 400+20;

my $w_Mavlink= Win32::GUI::DialogBox->new( -name=> 'mavlink_Window',  -parent => $w_Main, -font=> $StdWinFont,
    -text=> "Mavlink RC Tool",
    -pos=> [$MavlinkXPos,$MavlinkYPos],
    -size=> [$MavlinkXsize,$MavlinkYsize],
   -helpbox => 0,
);
$w_Mavlink->SetIcon($Icon);

sub t_MavlinkTool_Click{ MavlinkInit(); MavlinkShow(); 1; }
sub mavlink_Window_Terminate{ $w_Mavlink->Hide(); 0; }

my $MavlinkIsInitialized = 0;

sub MavlinkInit{
  if( $MavlinkIsInitialized>0 ){ return; }
  $MavlinkIsInitialized = 1;
  my $xpos= 20;
  my $ypos= 20;
  $w_Mavlink->AddButton( -name=> 'mavlink_PitchUp', -font=> $StdWinFont,
    -text=> 'PitchUp', -pos=> [$xpos,$ypos-3], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."PitchUp"."\r\n" ); SendMavlinkCmd( '0A' . 'E803' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_PitchMid', -font=> $StdWinFont,
    -text=> 'PitchMid', -pos=> [$xpos,$ypos-3+25], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."PitchMid"."\r\n" ); SendMavlinkCmd( '0A' . 'DC05' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_PitchDown', -font=> $StdWinFont,
    -text=> 'PitchDown', -pos=> [$xpos,$ypos-3+50], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."PitchDown"."\r\n" ); SendMavlinkCmd( '0A' . 'D007' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_PitchReCenter', -font=> $StdWinFont,
    -text=> 'PitchReCenter', -pos=> [$xpos,$ypos-3+75], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."PitchReCenter"."\r\n" ); SendMavlinkCmd( '0A' . '0000' ); 1; },
  );
  $xpos+= 120;
  $w_Mavlink->AddButton( -name=> 'mavlink_RollUp', -font=> $StdWinFont,
    -text=> 'RollUp', -pos=> [$xpos,$ypos-3], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."RollUp"."\r\n" ); SendMavlinkCmd( '0B' . 'E803' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_RollMid', -font=> $StdWinFont,
    -text=> 'RollMid', -pos=> [$xpos,$ypos-3+25], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."RollMid"."\r\n" ); SendMavlinkCmd( '0B' . 'DC05' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_RollDown', -font=> $StdWinFont,
    -text=> 'RollDown', -pos=> [$xpos,$ypos-3+50], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."RollDown"."\r\n" ); SendMavlinkCmd( '0B' . 'D007' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_RollReCenter', -font=> $StdWinFont,
    -text=> 'RollReCenter', -pos=> [$xpos,$ypos-3+75], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."RollReCenter"."\r\n" ); SendMavlinkCmd( '0B' . '0000' ); 1; },
  );
  $xpos+= 120;
  $w_Mavlink->AddButton( -name=> 'mavlink_YawUp', -font=> $StdWinFont,
    -text=> 'YawUp', -pos=> [$xpos,$ypos-3], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."YawUp"."\r\n" ); SendMavlinkCmd( '0C' . 'E803' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_YawMid', -font=> $StdWinFont,
    -text=> 'YawMid', -pos=> [$xpos,$ypos-3+25], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."YawMid"."\r\n" ); SendMavlinkCmd( '0C' . 'DC05' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_YawDown', -font=> $StdWinFont,
    -text=> 'YawDown', -pos=> [$xpos,$ypos-3+50], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."YawDown"."\r\n" ); SendMavlinkCmd( '0C' . 'D007' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_YawReCenter', -font=> $StdWinFont,
    -text=> 'YawReCenter', -pos=> [$xpos,$ypos-3+75], -width=> 100,
    -onClick=> sub{ TextOut( "\r\n"."YawReCenter"."\r\n" ); SendMavlinkCmd( '0C' . '0000' ); 1; },
  );
  $xpos+= 120;
  $w_Mavlink->AddButton( -name=> 'mavlink_SetAngle', -font=> $StdWinFont,
    -text=> 'SetAngle', -pos=> [$xpos,$ypos-3], -width=> 100,
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetAngle_Pitch', -font=> $StdWinFont,
    -text=> '0', -pos=> [$xpos,$ypos-3+25], -size=> [100,23],
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetAngle_Roll', -font=> $StdWinFont,
    -text=> '0', -pos=> [$xpos,$ypos-3+50], -size=> [100,23],
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetAngle_Yaw', -font=> $StdWinFont,
    -text=> '0', -pos=> [$xpos,$ypos-3+75], -size=> [100,23],
  );
  $w_Mavlink->AddCheckbox( -name=> 'mavlink_SetAngle_UnLimited', -font=> $StdWinFont,
    -text=> 'unlimited', -pos=> [$xpos,$ypos-3+100], -size=> [100,23],
  );

  $xpos= 20;
  $ypos= 20 + 50+ 25+25+20+20;
  $w_Mavlink->AddButton( -name=> 'mavlink_AuxKeyMode0', -font=> $StdWinFont,
    -text=> 'ActivePanMode Default', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Aux Key off-off"."\r\n" ); SendMavlinkCmd( '64' . '00' ); 1; },
  );
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_AuxKeyMode1', -font=> $StdWinFont,
    -text=> 'ActivePanModeSetting #1', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Aux Key off-on"."\r\n" ); SendMavlinkCmd( '64' . '01' ); 1; },
  );
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_AuxKeyMode2', -font=> $StdWinFont,
    -text=> 'ActivePanModeSetting #2', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Aux Key on-off"."\r\n" ); SendMavlinkCmd( '64' . '02' ); 1; },
  );
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_AuxKeyMode3', -font=> $StdWinFont,
    -text=> 'ActivePanModeSetting #3', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Aux Key on-on"."\r\n" ); SendMavlinkCmd( '64' . '03' ); 1; },
  );
  $xpos+= 160;
  $ypos= 20 + 50+ 25+25+20+20;
  $w_Mavlink->AddButton( -name=> 'mavlink_SetStandBy', -font=> $StdWinFont,
    -text=> 'Set StandBy', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Set StandBy"."\r\n" ); SendMavlinkCmd( '0E' . '01' ); 1; },
  );
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_ResetStandBy', -font=> $StdWinFont,
    -text=> 'Reset StandBy', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Reset StandBy"."\r\n" ); SendMavlinkCmd( '0E' . '00' ); 1; },
  );
  $xpos+= 160;
  $ypos= 20 + 50+ 25+25+20+20;
  $w_Mavlink->AddButton( -name=> 'mavlink_CameraShutter', -font=> $StdWinFont,
    -text=> 'Shutter', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Shutter"."\r\n" ); SendMavlinkCmd( '0F'. 'aa'.'01'.'aaaaaaaa' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_CameraShutterDelayed', -font=> $StdWinFont,
    -text=> 'Shutter Delayed', -pos=> [$xpos,$ypos-3+25], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Shutter Delayed"."\r\n" ); SendMavlinkCmd( '0F'. 'aa'.'02'.'aaaaaaaa' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_CameraVideoOn', -font=> $StdWinFont,
    -text=> 'Video On', -pos=> [$xpos,$ypos-3+50], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Video On"."\r\n" ); SendMavlinkCmd( '0F'. 'aa'.'03'.'aaaaaaaa' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_CameraVideoOff', -font=> $StdWinFont,
    -text=> 'Video Off', -pos=> [$xpos,$ypos-3+75], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Video Off"."\r\n" ); SendMavlinkCmd( '0F'. 'aa'.'04'.'aaaaaaaa' ); 1; },
  );
  $w_Mavlink->AddButton( -name=> 'mavlink_CameraReset', -font=> $StdWinFont,
    -text=> 'Reset', -pos=> [$xpos,$ypos-3+100], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Camera Reset"."\r\n" ); SendMavlinkCmd( '0F'. 'aa'.'00'.'aaaaaaaa' ); 1; },
  );
  $xpos= 20;
  $ypos= 20 + 50+ 25+25+20+20 + 100 + 20;
  $w_Mavlink->AddButton( -name=> 'mavlink_GetVersion', -font=> $StdWinFont,
    -text=> 'Get Version', -pos=> [$xpos,$ypos-3], -width=> 140,
    -onClick=> sub{ TextOut( "\r\n"."Get Version"."\r\n" ); SendMavlinkCmd( '01', 8+2*3 ); 1; },
  );
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_GetParameter', -font=> $StdWinFont,
    -text=> 'Get Parameter', -pos=> [$xpos,$ypos-3], -width=> 140,
  );
  $w_Mavlink->AddLabel( -name=> 'mavlink_GetParameter_label', -font=> $StdWinFont,
    -text=> "Nr", -pos=> [$xpos+150,$ypos],
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_GetParameter_Nr', -font=> $StdWinFont,
    -pos=> [$xpos+150+$w_Mavlink->mavlink_GetParameter_label->Width()+3,$ypos-3], -size=> [40,23],
  );
  $w_Mavlink->mavlink_GetParameter_Nr->Text(0);
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_SetParameter', -font=> $StdWinFont,
    -text=> 'Set Parameter', -pos=> [$xpos,$ypos-3], -width=> 140,
  );
  $w_Mavlink->AddLabel( -name=> 'mavlink_SetParameterNr_label', -font=> $StdWinFont,
    -text=> "Nr", -pos=> [$xpos+150,$ypos],
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetParameter_Nr', -font=> $StdWinFont,
    -pos=> [$xpos+150+$w_Mavlink->mavlink_SetParameterNr_label->Width()+3,$ypos-3], -size=> [40,23],
  );
  $w_Mavlink->mavlink_SetParameter_Nr->Text(0);
  $w_Mavlink->AddLabel( -name=> 'mavlink_SetParameterValue_label', -font=> $StdWinFont,
    -text=> "Value", -pos=> [$xpos+210,$ypos],
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetParameter_Value', -font=> $StdWinFont,
    -pos=> [$xpos+210+$w_Mavlink->mavlink_SetParameterValue_label->Width()+3,$ypos-3], -size=> [40,23],
  );
  $w_Mavlink->mavlink_SetParameter_Value->Text(0);
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_GetData', -font=> $StdWinFont,
    -text=> 'Get Data', -pos=> [$xpos,$ypos-3], -width=> 140,
#    -onClick=> sub{ TextOut( "\r\n"."Get Data"."\r\n" ); SendMavlinkCmd( '05'. '00', 74 ); 1; }, # 8 +2 + 2*32 = 74
    -onClick=> sub{
      TextOut( "\r\n"."Get Data"."\r\n" ); SendMavlinkCmd( '05'. '00', 8+2+2*$CMD_d_PARAMETER_ZAHL);
      1; }, # 8 +2 + 2*32 = 74
  );
  ...
  $ypos= 20 + 50+ 25+25+20+20 + 100 + 20;
  $ypos+= 25;
  $w_Mavlink->AddButton( -name=> 'mavlink_SetPwmOut', -font=> $StdWinFont,
    -text=> 'SetPwmOut', -pos=> [$xpos,$ypos-3], -width=> 140,
  );
  $w_Mavlink->AddTextfield( -name=> 'mavlink_SetPwmOut_Value', -font=> $StdWinFont,
...
sub mavlink_SetAngle_Click{
  TextOut( "\r\n".'SetAngle'."\r\n" );
  my $pitch = sprintf("%.5E",$w_Mavlink->mavlink_SetAngle_Pitch->Text());
  my $roll = sprintf("%.5E",$w_Mavlink->mavlink_SetAngle_Roll->Text());
  my $yaw = sprintf("%.5E",$w_Mavlink->mavlink_SetAngle_Yaw->Text());
  ...
  my $flags = '00'; #angles are limited
  if( $w_Mavlink->mavlink_SetAngle_UnLimited->GetCheck()>0 ){ $flags= '07'; } # all angles are unlimited
  my $type = '00'; # type is proper gimbal Euler angles in STorM32 frame (NWU)
#TextOut('!'.$pitchf.','.$rollf.','.$yawf.','.$flags.','.$type.'!');
  SendMavlinkCmd( '11' . $pitchf . $rollf . $yawf . $flags . $type );
...
sub mavlink_GetParameter_Click{
  TextOut( "\r\n".'Get Parameter'."\r\n" );
  my $s= $w_Mavlink->mavlink_GetParameter_Nr->Text();
  #TextOut( UCharToHexstr($s) );
  SendMavlinkCmd( '03'. UCharToHexstr($s).'00', 8+2*2 );
  1;
}

sub mavlink_SetParameter_Click{
  TextOut( "\r\n".'Set Parameter'."\r\n" );
  my $s= $w_Mavlink->mavlink_SetParameter_Nr->Text();
  $s= UCharToHexstr($s).'00';
  TextOut( $s.'!' );
  my $v= $w_Mavlink->mavlink_SetParameter_Value->Text();
  if( $v<0 ){ $v = $v+65536; }
  $v= UIntToHexstr($v);
  $v= substr($v,2,2).substr($v,0,2);
  TextOut( $v.'!' );
...
sub mavlink_SetPwmOut_Click{
  TextOut( "\r\n".'SetPwmOut'."\r\n" );
  my $v= $w_Mavlink->mavlink_SetPwmOut_Value->Text();
  TextOut( $v.'!' );
  $v= UIntToHexstr($v);
  $v= substr($v,2,2).substr($v,0,2);
  TextOut( $v.'!' );
...
sub SendNativeMavlinkCmd{
  ...
  my $msglen = UCharToHexstr( length($msg)/2 - 1);
  my $cmd = $MAVSTX. $msglen .'00'.'52'.'43'. $msg . '33'.'34'; #crc check is not activated, hence dummy crc
...
sub MotionControlInit{
  ...
  $w_MotionControl-> AddTextfield( -name=> 'mcontrol_Text', -font=> $MotionControlEditorTextFont, #-font=> $StdWinFont,
    -pos=> [-1,-1+15], -size=> [$MotionControlXsize-4,$MotionControlYsize-42-15], #-size=> [$xsize-14,$ysize-52],
    -hscroll=> 1, -vscroll=> 1,
    -autovscroll=> 1, -autohscroll=> 1,
    -keepselection => 1,
    -multiline=> 1,
...
#  if( not ConnectionIsValid() ){ return 0; }
  ...
  my $c = eval($w_MotionControl->mcontrol_Text->Text());
  #die $@ if ($@);
...
TextOut( 'CMD_DOCAMERA '.$cmd."\n" );
  ...
  SendMavlinkCmd( '0F'. 'aa'.$data.'aaaaaaaa' );
}

sub DoCamera{ CMD_DOCAMERA(shift); }


#pitch, roll, yaw, flags, type
sub CMD_SETANGLE{
  my $pitch = shift;
  my $roll = shift;
  my $yaw = shift;
...
##  TextOut( $pitch.','.$roll.','.$yaw.',0,0!' );
  ...
  my $yawf = FloatToHexstrSwapped($yaw);
  my $flags = '00'; #angles are limited
  $flags= '07';  # all angles are unlimited
  my $type = '00'; # type is proper gimbal Euler angles in STorM32 frame (NWU)
#TextOut('!'.$pitchf.','.$rollf.','.$yawf.','.$flags.','.$type.'!');
  SendMavlinkCmd( '11' . $pitchf . $rollf . $yawf . $flags . $type );
}

sub SetAngle{ CMD_SETANGLE(shift,shift,shift); }
...
sub SetPwmOut{ CMD_SETPWMOUT(shift); }
...
sub CMD_SETPARAMETER{
  my $nr = shift; my $value = shift;
TextOut( 'CMD_SETPARAMETER '.$nr.','.$value."\n" );
  ...
  $v= UIntToHexstr($v);
  $v= substr($v,2,2).substr($v,0,2);
  SendMavlinkCmd( '04'. $s.$v );
}

sub SetParameter{
  my $nrstr = shift; my $value = shift;
  my $nr;
  if( $nrstr =~ /^\d{1,3}$/  ){ #consider this to be a number
    ...
    if( $nr eq '' ){ #parametername is not known, throw an error
      TextOut( 'ERROR: SetParamter('.$nrstr.','.$value.')'."\n" );
      return;
    }
  }
  CMD_SETPARAMETER( $nr, $value );
}
