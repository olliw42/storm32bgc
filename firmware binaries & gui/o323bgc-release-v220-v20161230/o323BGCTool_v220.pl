...
my $VersionStr= '30. Dez. 2016 v2.20';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '219' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()

my @FirmwareVersionList=  ( 'v2.20 NT','v2.19e NT','v2.03 NT','v0.96 NT' ); #versions available in the flash tab selector
my @FirmwareVReplaceList= ( 'v220_nt','v219e_nt','v203_nt','v096_nt' ); #versions available in the flash tab selector

my @STorM32BoardList= (
{
  name => 'STorM32 v2.4',             hexfile => 'o323bgc_V_storm32bgc_v240_f103rc',
},{
  ...
  name => 'NT Imu Module v2.1',  hexfile => 'o323bgc_ntimu_V_module_v21_f103t8',
  versions =>  [ 'v0.31' ],
  vreplaces => [ 'v031' ],
  boards =>    [ 'v2.1 F103T8' ],
},{
  name => 'NT Imu Module v1.x',  hexfile => 'o323bgc_ntimu_V_module_v10_f103t8',
  versions =>  [ 'v0.31', 'v0.29' ],
  vreplaces => [ 'v031', 'v029' ],
  ...
  versions =>  [ 'v0.31' ],
  vreplaces => [ 'v031' ],
  boards =>    [ 'v1.1 F103T8' , 'v1.0 F103T8','v2.2E F103T8','v2.3E F103T8'], #the last ones are dummies to bypass the hardware check
},{
  name => 'NT Logger Module v1.x',    hexfile => 'o323bgc_ntlogger_V_module_v10_f103t8',
  versions =>  [ 'v0.26' ],
  vreplaces => [ 'v026' ],
  boards =>    [ 'v1.0 F103T8','v1.1 F103T8' ],
},{
  name => 'NT Imu Module CC3D Atom',  hexfile => 'o323bgc_ntimu_V_module_cc3datom_f103cb',
  versions =>  [ 'v0.31', 'v0.29' ],
  vreplaces => [ 'v031', 'v029' ],
  boards =>    [ 'CC3D F103CB' ],
},{
  name => 'NT Imu Module v1.x as Imu3',  hexfile => 'o323bgc_ntimu_V_module_v10_f103t8_asimu3',
  versions =>  [ 'v0.31', 'v0.29' ],
  vreplaces => [ 'v031', 'v029' ],
  boards =>    [ 'v1.0 F103T8','v1.1 F103T8' ],
...
"When you're updating from v0.96, v0.96 nt, v2.03 nt or later, then you just need to flash the new firmware. ".
"All settings, including the scripts and calibration data, are copied over.

For older versions, it depends. Expect that this firmware will reset all parameters, scripts and calibration settings when flashed. ".
"Please memorize your old settings before flashing, and restore them afterwards manually.

The NT imu and NT motor module firmwares were modified; please upgrade. The firmwares of other NT module were not modified.

Please note that, for reasons of size, the NTLoggerTool is NOT included in the standard package! ".
...
my @SetupTabList= ( 'main', 'pid', 'pan', 'rcinputs', 'functions', 'scripts', 'gimbalsetup',
                    'gimbalconfig', 'expert', 'betacopter' );
my $MaxSetupTabs= scalar(@SetupTabList); #9; #this is the number of setup tabs
...
##--- PID tab --------------------
  ...
  name => 'Pitch I',
  type => 'OPTTYPE_UI', len => 7, ppos => 1, min => 0, max => 32000, default => 1000, steps => 50,
  size => 2,
  adr => 1,
},{
  name => 'Pitch D',
...
##--- PAN tab --------------------
  ...
  type => 'OPTTYPE_UI', len => 5, ppos => 2, min => 0, max => 300, default => 150, steps => 5,
  size => 2,
  adr => 118,
  unit=> 's',
...
##--- FUNCTIONS tab --------------------
  ...
  adr => 70,
  choices => \@FunctionInputChoicesList,
  expert=> GetTabIndex('functions'), #4,
  column=> 1,
...
##--- GIMBAL SETUP tab --------------------
  ...
  expert=> GetTabIndex('gimbalsetup'), #5,
  ...
  name => 'Startup Delay',
  type => 'OPTTYPE_UI', len => 0, ppos => 1, min => 0, max => 250, default => 0, steps => 5,
  size => 2,
  adr => 31,
  unit => 's',
},{
  ...
  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 11, default => 0, steps => 1,
  size => 1,
  adr => 41,
...
##--- CONFIGURE GIMBAL tab  --------------------
  ...
  expert=> GetTabIndex('gimbalconfig'), #7,
  pos=>[1,1],
},{
...
##--- EXPERT tab  --------------------
  ...
  type => 'OPTTYPE_UI', len => 0, ppos => 1, min => 0, max => 250, default => 50, steps => 5,
  size => 2,
  adr => 30,
  unit => 's',
...
my $Storm32UpgradeViaSystemBootloader= 'Upgrade via System Bootloader @ UART1';
my $NtUpgradeViaUSB= 'Upgrade via STorM32 USB port';
my $UpgradeViaSystemBootloader= 'Upgrade via System Bootloader @ UART1';
#my @Storm32ProgrammerList= ( $StLink, $SystemBootloader );
my @Storm32ProgrammerList= ( $Storm32UpgradeViaSystemBootloader, $StLink, $SystemBootloader );
my @NtProgrammerList= ( $NtUpgradeViaUSB, $UpgradeViaSystemBootloader, $StLink, $SystemBootloader );
...
###############################################################################
#-----------------------------------------------------------------------------#
my @BetaCopterTool = ( ">BetaCopter Tool...", 't_BetaCopterTool', );
...
$w_Main= Win32::GUI::Window->new( -name=> 'm_Window', -font=> $StdWinFont,
  -text=> 'OlliW\'s '.$BGCStr.'Tool for STorM32-NT', -size=> [$xsize,$ysize], -pos=> [$DialogXPos,$DialogYPos],
  -menu=> $m_Menubar,
...
#$f_Tab{betacopter}= $w_Main->w_Tab->InsertItem(-text=> '    BetaCopter');
...
$f_Tab{betacopter}= Win32::GUI::DialogBox->new( -name=> 'm_BetaCopterWindow', -parent => $w_Main,
  -text=>'o323BGC BetaCopter Tool',
  -size=> [10+4*$OPTIONSWIDTH_X,360-2], -pos=> [$DialogXPos+$xsize-380-180,$DialogYPos+50], -helpbox => 0,
);
$f_Tab{betacopter}->Hide();

sub t_BetaCopterTool_Click{
  my ($x,$y)=($w_Main->GetWindowRect())[0..1];
  $f_Tab{betacopter}->Move($x+$ColNumber*$OPTIONSWIDTH_X+40-15+20-385-180+1-180,$y+50-12);
  $f_Tab{betacopter}->Show(); 0;
}
sub m_BetaCopterWindow_Terminate{ $f_Tab{betacopter}->Hide(); 0; }
...
sub UseSimplifiedPID_Click{
  if( $f_Tab{pid}->UseSimplifiedPID->Checked() ){
    ...
    $f_Tab{pid}->UseSpecialAlgorithm->Disable(); #changing special algorithm while in simplified can have nasty effects
  }else{
    $f_Tab{pid}->UseSimplifiedPID_frame->Hide();
    ...
    $f_Tab{pid}->UseSpecialAlgorithm->Enable();
  }
  1;
}
...
#STorM32 UpgradeViaSystemBootLoaderAtUart1 for V1x
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerUsage_Storm32v1x_UpgradeViaSysBootLoad_label', -font=> $StdWinFont,
  -pos=> [$xpos+430+35,$ypos+30], -multiline => 1, -height=>6*13+50, -width=>300,
  -text=> 'Usage:
1. Connect STorM32 board via usb
2. Select port of the STorM32 board
3. Connect usb adapter to UART1 at RC port
4. Select com port of the usb adapter
...
#STorM32 UpgradeViaSystemBootLoaderAtUart1 for V2x
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerUsage_Storm32v2x_UpgradeViaSysBootLoad_label', -font=> $StdWinFont,
  -pos=> [$xpos+430+35,$ypos+30], -multiline => 1, -height=>6*13+50, -width=>300,
  -text=> 'Usage:
1. Connect usb adapter to UART1
2. Select com port of the usb adapter
3. Power up STorM32 board
4. Hit >Flash Firmware<',
);
...
my $BOARDTYPE_IS_STORM32_V2X = 2;
my $BOARDTYPE_IS_NT = 1;
...
sub GetBoardTypeAndName{
  ...
  if( $boardname =~ /^STorM32 v2./ ){ $boardtype = $BOARDTYPE_IS_STORM32_V2X; }
  if( $boardname =~ /^NT/ ){ $boardtype = $BOARDTYPE_IS_NT; }
...
sub SetFirmwareHexFile{
  ...
  if( $boardtype == $BOARDTYPE_IS_NT ){ #this is a NT module
...
sub SetProgrammer{
  ...
  if( $pstr eq $SystemBootloader ){
    ...
    if( $boardtype == $BOARDTYPE_IS_NT ){ #this is a NT module
      ...
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_UpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_UpgradeViaSysBootLoad_label->Hide();
    ...
    }elsif( $boardtype == $BOARDTYPE_IS_STORM32_V2X ){ #this is a v2.x board
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_SysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_SysBootLoad_label->Show(); #THIS IS IT
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_UpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_UpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaStorm32Usb_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaSysBootLoad_label->Hide();
  ...
  }elsif( $pstr eq $UpgradeViaSystemBootloader ){
    $f_Tab{flash}->flash_ProgrammerComPort_label->Show();
    $f_Tab{flash}->flash_ProgrammerComPort->Show();
    $f_Tab{flash}->flash_ProgrammerModuleId_label->Hide();
    $f_Tab{flash}->flash_ProgrammerModuleId->Hide();
    if( $boardtype == $BOARDTYPE_IS_NT ){ #this is a NT module
      ...
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_SysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_UpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_UpgradeViaSysBootLoad_label->Show(); #THIS IS IT
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaStorm32Usb_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtSysBootLoad_label->Hide();
    }else{
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_SysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_SysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_UpgradeViaSysBootLoad_label->Show(); #THIS IS IT
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v2x_UpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaStorm32Usb_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaSysBootLoad_label->Hide();
      $f_Tab{flash}->flash_ProgrammerUsage_NtSysBootLoad_label->Hide();
...
sub SetRemoveProtections{
  ...
  if(( $f_Tab{flash}->flash_FullErase_check->GetCheck() )and
     ( ($pstr eq $SystemBootloader)or($pstr eq $UpgradeViaSystemBootloader) )){
    $f_Tab{flash}->flash_RemoveProtections_check->Checked(0);
...
sub flash_ScanNtBus_button_Click{
  ...
  my %ScanList = ( '01' => 'Imu1', '02' => 'Imu2',
                   '04' => 'Motor Pitch', '05' => 'Motor Roll', '06' => 'Motor Yaw',
                   '0B' => 'Logger',
                   '0C' => 'Imu3',
                   );
  for my $id (sort keys %ScanList){
    _delay_ms(20);
    TextOut_( "\r\n".'  '.TrimStrToLength($ScanList{$id},13) );
    ...
    if( length($test)==0 ){
      ...
      if( CheckStatus($config,$NTBUS_IMU_CONFIG_MPU6050) ){ TextOut_('MPU6050 '); }
      if( CheckStatus($config,$NTBUS_IMU_CONFIG_MPU6000) ){ TextOut_('MPU6000 '); }
      if( CheckStatus($config,$NTBUS_IMU_CONFIG_MPU9250) ){ TextOut_('MPU9250 '); }
...
##pre-checks
  # pre-checks for flash NT module using UpgradeViaUSB
  # check for Port is usb
  # let the user confirm the settings
  if( ($boardtype == $BOARDTYPE_IS_NT) and ($pstr eq $NtUpgradeViaUSB) ){
    $Port = $w_Main->m_Port->Text();
    if( not ComIsUSB($Port) ){
  ...
  # pre-chek, these need the connection to the STorM32 board via the normal Port
  if( (($boardtype == $BOARDTYPE_IS_NT) and ($pstr eq $NtUpgradeViaUSB)) or
      (($boardtype == $BOARDTYPE_IS_STORM32) and ($pstr eq $UpgradeViaSystemBootloader)) ){
    if( not ConnectionIsValid() ){
      if( not OpenPort() ){
...
##flash using SystemBootloader, in one way or the other
  # $SystemBootloader or $UpgradeViaSystemBootloader or $NtUpgradeViaUSB
  }else{
    #open the correct port, i.e. the one called by StmFlashLoader.exe
    # $SystemBootloader or $UpgradeViaSystemBootloader => ComPort
    # $NtUpgradeViaUSB => normal Port
    my $StmFlashLoaderPort;
    if( ($pstr eq $SystemBootloader) or ($pstr eq $UpgradeViaSystemBootloader) ){
      #these use the Com Port
      # STorM32 V1x UpgradeViaSystemBootloader needs the connection via the normal Port later
      # so we should not disconnect here, we will disconnected later
      if( not (($boardtype == $BOARDTYPE_IS_STORM32) and ($pstr eq $UpgradeViaSystemBootloader)) ){
...
##        TextOut( "\r\nDISCONNECTED" );
    ...
    }elsif( ($boardtype == $BOARDTYPE_IS_NT) and ($pstr eq $NtUpgradeViaUSB) ){
      #this uses the normal Port
      # NT UpgradeViaUsb needs the connection via the normal Port later
      # so we should not disconnect here, we will disconnected later
    ...
    if( ($boardtype == $BOARDTYPE_IS_NT) and ($pstr eq $NtUpgradeViaUSB) ){
      ...
      WritePort( 'cNTFLASH'.$id2 );
      SetDoFirstReadOut(0);
      DisconnectFromBoard(0); #this would happen anyway during a flash
      TextOut( "\r\nDISCONNECTED" );

    }elsif( ($boardtype == $BOARDTYPE_IS_NT) and ($pstr eq $UpgradeViaSystemBootloader) ){
...
#        for(my $ii=0;$ii<10000;$ii++){;}
        ...
        _delay_ms_com($p_ntbus,100);
        $p_ntbus->close;
      }

    }elsif( ($boardtype == $BOARDTYPE_IS_STORM32_V2X) and ($pstr eq $UpgradeViaSystemBootloader) ){
      #send first xQSFLASH to set STorM32 board into bootloader mode
      TextOut( "\r\nset STorM32 v2.x board into bootloader mode" );
      my $p_ntbus = Win32::SerialPort->new( $StmFlashLoaderPort );
      if( (defined $p_ntbus) and ($p_ntbus) ){
        $p_ntbus->baudrate(115200);
        configport_com($p_ntbus);
        $p_ntbus->owwrite_overlapped_undef( 'xQscFLASH' );
        _delay_ms_com($p_ntbus,100);
        $p_ntbus->close;
      }

    }elsif( ($boardtype == $BOARDTYPE_IS_STORM32) and ($pstr eq $UpgradeViaSystemBootloader) ){
      #send first xQSFLASH to set STorM32 board into bootloader mode
      TextOut( "\r\nset STorM32 board into bootloader mode" );
      my $s = ExecuteCmd( 'xQS' );
#TextOut( "!".$s."!" );
      if( substr($s,length($s)-1,1) ne 'o' ){
        TextOut( "\r\nFlash firmware... ABORTED!\r\nConnection to STorM32 board failed!\r\n" ); return 1;
...
sub _delay_ms_com{
  my $com = shift;
  my $tmo= shift;
  $tmo+= $com->get_tick_count(); #timeout in ms
  do{ }while( $com->get_tick_count()< $tmo );
}
...
sub ExtractStatusInfo{
  ...
  my $status3 = '00000000'.substr($state,0,8); #status3 = high byte of state shifted to low byte
...
sub Mag_CheckBoxes_Enable{
  ...
  #$w_DataDisplay->dd_PlotMagYaw_check->Show($flag);
}
...
#  TextOut( $w_DataDisplay->dd_PlotC->Top()."\r\n"); #325 @535
  ...
  $w_DataDisplay->dd_PlotCntrlRoll_label->Top( $mh - (535-365) ); #365 @535
  $w_DataDisplay->dd_PlotCntrlYaw_label->Top( $mh - (535-385) ); #385 @535
...
#  TextOut( $w_DataDisplay->dd_LipoVoltage_label->Top()."\r\n"); #470 @535
  ...
  $w_DataDisplay->dd_I2CError_label->Top( $mh - 65 );
  $w_DataDisplay->dd_I2CError->Top( $mh - 65 );
  ...
  $w_DataDisplay->dd_MagStatus_label->Top( $mh - 65 );
  $w_DataDisplay->dd_MagStatus->Top( $mh - 65 );
...
#  TextOut( "?".$packedfunctionvalues);
  ...
  $DataMatrix[$DataAabs_i][$DataPos]= 10000.0/8192.0 * $Aabs; #XX BUG: use $AccGravityConst????
...
my $RcCmdUse0xFE = $MavlinkRcUse0xFE;
...
my $RC_CMD_GETVERSION = '01';
my $RC_CMD_GETVERSION_RESPONSE_LEN = 11;
my $RC_CMD_GETVERSIONSTR = '02';
my $RC_CMD_GETVERSIONSTR_RESPONSE_LEN = 3 + 3*16 + 2;
my $RC_CMD_GETPARAMETER = '03';
my $RC_CMD_GETPARAMETER_RESPONSE_LEN = 9;
my $RC_CMD_GETDATA = '05';
my $RC_CMD_GETDATA_RESPONSE_LEN = 3 + 2+2*$CMD_d_PARAMETER_ZAHL + 2;
my $RC_CMD_GETDATAFIELDS = '06'; #length is variable!!!
my $RC_CMD_ACK_LEN = 6;
...
#TextOut( "!$rc_cmd_id!" );
    if( $rc_cmd_id eq $RC_CMD_GETPARAMETER  ){
      $doread = $RC_CMD_GETPARAMETER_RESPONSE_LEN;
    }elsif( $rc_cmd_id eq $RC_CMD_GETVERSION  ){
      $doread = $RC_CMD_GETVERSION_RESPONSE_LEN;
    }elsif( $rc_cmd_id eq $RC_CMD_GETVERSIONSTR  ){
      $doread = $RC_CMD_GETVERSIONSTR_RESPONSE_LEN;
    }elsif( $rc_cmd_id eq $RC_CMD_GETDATA  ){
      $doread = $RC_CMD_GETDATA_RESPONSE_LEN;
    }elsif( $rc_cmd_id eq $RC_CMD_GETDATAFIELDS  ){
      if($DetailsOut){ TextOut( 'No length specified, RC command is ignored!'."\r\n" ); }
    }else{
      $doread = $RC_CMD_ACK_LEN; #6 is is the default CMD_ACK
    }
#TextOut( "!$doread!" );
  }
...
sub SendEmbeddedMavlinkCmd{
  my $msg = shift;
  my $msglen = UCharToHexstr( length($msg)/2 - 1 ); #don't count the msg_id byte
  my $rccmd = $RCCMDINSTX . $msglen . $msg; #msg without crc
