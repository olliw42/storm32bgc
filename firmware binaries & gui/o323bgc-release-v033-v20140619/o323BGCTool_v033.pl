...
my $VersionStr= '19. June 2014 v0.33';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '33' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()
...
my @FirmwareVersionList= ( 'v0.33','v0.31','v0.29','v0.28','v0.25e','v0.24','v0.15','v0.14' ); #versions available in the flash tab selector
...
#  pos=>[1,3],
  ...
  pos=>[1,5],
#},{
#  name => 'PWM Passthrough Configuration',
#  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 2, default => 0, steps => 1,
#  size => 1,
#  adr => 42,
#  choices => [ 'off',  'pwm 1520us 55Hz' ],
#  expert=> 1,
#  pos=>[2,5],
...
##--- RC INPUTS tab --------------------
  ...
  name => 'Rc Pitch',
  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 32, default => 0, steps => 1,
  size => 1,
  adr => 44,
  choices => [ 'off', 'Rc-0', 'Rc-1', 'Rc-2', 'Rc2-0', 'Rc2-1', 'Rc2-2', 'Rc2-3', 'Pot-0', 'Pot-1', 'Pot-2',
               ...
               'But press', 'But push',
               'Aux-0 press', 'Aux-1 press', 'Aux-2 press', 'Aux-01 press', 'Aux-012 press',
               'Aux-0 push', 'Aux-1 push', 'Aux-2 push', 'Aux-01 push', 'Aux-012 push' ],
  column => 2,
  expert=> 3,
},{
  name => 'Rc Pitch Mode',
...
##--- EXPERT tab  --------------------
  ...
  name => 'Acc Compensation Method (e)',
  type => 'OPTTYPE_LISTA', len => 0, ppos => 0, min => 0, max => 1, default => 0, steps => 1,
  size => 1,
  adr => 88,
  ...
  name => 'Acc Noise Level (e)',
  type => 'OPTTYPE_UI', len => 0, ppos => 3, min => 0, max => 150, default => 10, steps => 1,
  size => 2,
  adr => 89,
  unit => 'g',
  expert=> 2,
},{
  name => 'Acc Threshold (0 = off) (e)',
  type => 'OPTTYPE_UI', len => 0, ppos => 2, min => 0, max => 100, default => 25, steps => 1,
  size => 2,
  adr => 90,
  unit => 'g',
  expert=> 2,
},{
  name => 'Acc Vertical Weight (e)',
  ...
  name => 'Acc Zentrifugal Correction (e)',
  type => 'OPTTYPE_UI', len => 0, ppos => 0, min => 0, max => 100, default => 25, steps => 5,
  size => 2,
  adr => 92,
  unit => '%',
  ...
  name => 'Acc Recover Time (e)',
  type => 'OPTTYPE_UI', len => 0, ppos => 0, min => 0, max => 1000, default => 100, steps => 5,
  size => 2,
  adr => 93,
  unit => ' ms',
  expert=> 2,
...
my @SetupTabList= ( 'setup', 'gimbalsetup', 'expert', 'rcinputs', 'functions' ); #this is the name of the experts tabs
...
$f_Tab{rcinputs}->AddButton( -name=> 'm_rc_AutoTrim', -font=> $StdWinFont,
  -text=> 'Auto Trim', -pos=> [$xpos+0*$OPTIONSWIDTH_X,$ypos+4+5*$OPTIONSWIDTH_Y], -width=> 120,
...
$f_Tab{configimu}->AddLabel( -name=> 'mi_ImuOrientationText2_label', -font=> $StdWinFont,
  -text=> 'Check the setting by running the Data Display: Tilting the camera downwards must
...
sub m_Status_Click{ ExecuteGetStatus(); 1; }
...
sub ExecuteDefault{
  return ExecuteCommandFull( 'xd', 'Set to Default', '' );
}
...
sub ExecuteRetrieveFromEeprom{
  return ExecuteCommandFull( 'xr', 'Retrieve from EEPROM', '' );
}

sub ExecuteEraseEeprom{
  return ExecuteCommandFull( 'xc', 'Erase EEPROM', 'NOTE: Please reset or power up the BGC board for proper operation!' );
}

sub ExecuteLevelGimbal{
  return ExecuteCommandFull( 'xl', 'Level gimbal', '' );
}
...
sub ExecuteResetController{
  ...
  ExecuteCommandFull( 'xx', 'Reset BG controller', '' );
  #handle case if Data Display is running
...
sub ExecuteGetCurrentMotorDirections{
  return ExecuteCommandFull( 'xm', 'Get current motor directions', '' );
}

sub ExecuteGetCurrentMotorPositions{
  return ExecuteCommandFull( 'xz', 'Get current motor positions', '' );
}

sub ExecuteCalibrateRcTrim{
  $ExecuteCommandFullDelayBeforeGet= 1500;
  return ExecuteCommandFull( 'CR', 'Calibrate rc trims', '' );
}
