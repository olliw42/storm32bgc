...
my $VersionStr= '8. Apr. 2015 v2.02';
...
my @FirmwareVersionList=  ( 'v2.02 NT','v2.01 NT','v0.96','v0.96 NT' ); #versions available in the flash tab selector
my @FirmwareVReplaceList= ( 'v202_nt','v201_nt','v096','v096_nt' ); #versions available in the flash tab selector
...
my @STorM32BoardList= (
{
  ...
  name => 'NT Imu Module v1.0',       hexfile => 'o323bgc_ntimu_V_module_v10_f103t8',
  versions =>  [ 'v0.23' ],
  vreplaces => [ 'v023' ],
  boards =>    [ 'v1.0 F103T8' ],
},{
  name => 'NT Motor Module v1.0',     hexfile => 'o323bgc_ntmotor_V_module_v10_f103t8',
  versions =>  [ 'v0.22' ],
  vreplaces => [ 'v022' ],
  boards =>    [ 'v1.0 F103T8' ],
},{
  name => 'NT Logger Module v1.0',    hexfile => 'o323bgc_ntlogger_V_module_v10_f103t8',
  versions =>  [ 'v0.23' ],
  vreplaces => [ 'v023' ],
  boards =>    [ 'v1.0 F103T8' ],
},{
...
"When you're updating from v0.96 nt, v0.96, or v2.01, then you just need to flash the new firmware. ".
...
The NT imu and logger module firmwares got updated, please reflash these NT modules if you're using them.
...
my @Stm32ProgrammerList= ( $StLink, $SystemBootloader );
...
if( defined $IniFile ){
  ...
  if( defined $IniFile->val('FLASH','Stm32Programmer') ){ $Stm32Programmer= $IniFile->val('FLASH','Stm32Programmer'); }
...
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerStm32Usage_label', -font=> $StdWinFont,
...
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerNtUsageA_label', -font=> $StdWinFont,
...
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerNtUsageB_label', -font=> $StdWinFont,
  -pos=> [$xpos+430+35,$ypos+30], -multiline => 1, -height=>6*13+50, -width=>300,
  -text=> 'Usage:
...
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerNtUsageD_label', -font=> $StdWinFont,
  -pos=> [$xpos+430+35,$ypos+30], -multiline => 1, -height=>6*13+50, -width=>300,
  -text=> 'Usage:
1. Connect usb adapter to NT module
2. Select com port of the usb adapter
...
sub SetFirmwareHexFile{
  ...
  if( $boardtype == 1 ){ #this is a NT module
...
my $lastboardtype = -1; #this ensures that list is set up at startup
sub SetBoard{
  ...
  if( $boardtype != $lastboardtype ){ #type has been changed
    ...
    if( $boardtype == 1 ){ #this is a NT module
      ...
      $f_Tab{flash}->flash_Programmer->Add( @Stm32ProgrammerList );
      $f_Tab{flash}->flash_Programmer->SelectString( $Stm32Programmer );
    }
  }
  if( $boardtype == 1 ){ #this is a NT module
...
sub SetProgrammer{
  ...
  if( $pstr eq $SystemBootloader ){
    ...
    if( $boardtype == 1 ){ #this is a NT module
      $f_Tab{flash}->flash_ProgrammerStm32Usage_label->Hide();
      $f_Tab{flash}->flash_ProgrammerNtUsageD_label->Show();
    }else{
      $f_Tab{flash}->flash_ProgrammerStm32Usage_label->Show();
      $f_Tab{flash}->flash_ProgrammerNtUsageD_label->Hide();
    }
    $f_Tab{flash}->flash_ProgrammerNtUsageA_label->Hide();
    $f_Tab{flash}->flash_ProgrammerNtUsageB_label->Hide();

  }elsif( $pstr eq $NtUpgradeViaSystemBootloader ){
    $f_Tab{flash}->flash_ProgrammerComPort_label->Show();
    $f_Tab{flash}->flash_ProgrammerComPort->Show();
    $f_Tab{flash}->flash_ProgrammerModuleId_label->Hide();
    $f_Tab{flash}->flash_ProgrammerModuleId->Hide();
    $f_Tab{flash}->flash_ProgrammerStm32Usage_label->Hide();
    $f_Tab{flash}->flash_ProgrammerNtUsageA_label->Hide();
    $f_Tab{flash}->flash_ProgrammerNtUsageB_label->Show();
    $f_Tab{flash}->flash_ProgrammerNtUsageD_label->Hide();
    ...
    $f_Tab{flash}->flash_ProgrammerStm32Usage_label->Hide();
    $f_Tab{flash}->flash_ProgrammerNtUsageA_label->Show();
    $f_Tab{flash}->flash_ProgrammerNtUsageB_label->Hide();
    $f_Tab{flash}->flash_ProgrammerNtUsageD_label->Hide();
...
sub flash_ScanNtBus_button_Click{
  ...
  for my $id (sort keys %ScanList){
    ...
    if( length($test)==0 ){
      ...
      TextOut_( 'firmware: '.$firmware.'  board: '.$board );
    }
  }
  TextOut( "\r\nScan NT bus... DONE\r\n" );
EXIT:
...
#XX  DisconnectFromBoard(0); #this would happen anyway during a flash
...
##flash using SystemBootloader, in one way or the other
  }else{ #if( $pstr eq $SystemBootloader ){ #three cases can happen here
    #open the correct port
    my $portnr;
    if(( $pstr eq $SystemBootloader )or( $pstr eq $NtUpgradeViaSystemBootloader )){ #these need the com port
      SetDoFirstReadOut(0);
      DisconnectFromBoard(0); #this would happen anyway during a flash
      $portnr = ExtractCom( $f_Tab{flash}->flash_ProgrammerComPort->Text() );
      if( $portnr eq '' ){
        TextOut( "\r\nFlash firmware... ABORTED!\r\nCom port not specified!\r\n" ); return 1;
      }
      TextOut( "\r\nuse System Bootloader @ UART1" );
    }elsif( $pstr eq $NtUpgradeViaUSB ){ #this needs the normal port
      if( not ConnectionIsValid() ){
        if( not OpenPort() ){
    ...
    if( $pstr eq $NtUpgradeViaUSB ){
      ...
      #send first STX FLASH to set module into bootloader mode
...
$IniFile->newval( 'FLASH', 'Stm32Programmer', $Stm32Programmer );
