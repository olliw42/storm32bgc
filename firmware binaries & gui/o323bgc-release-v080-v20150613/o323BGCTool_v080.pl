...
my $VersionStr= '13. June 2015 v0.80';

#CAREFULL: in Data Display version is not double checked!
my @SupportedBGCLayoutVersions= ( '78' ); #layout versions supported by o32BGCTool, used in ExecuteHeader()
...
my @FirmwareVersionList= ( 'v0.80','v0.70','v0.68' ); #versions available in the flash tab selector
...
my $m_Menubar= Win32::GUI::Menu-> new(
  ...
  'Tools' => '',
    ...
    ">RC Command Tool... (experts only)", 't_RcCmdTool',
    @ExpertTool,
    ">-", 0,
...
#SetReadPortDebug(1);
#BUG  my ($ret,$s)= ExecuteCommandFullwoGet( 'Cr', 'Read calibration data', '', 18*2, 1 );
...
my $STATUS_NTBUS_INUSE              =  7; #0x0100

my $STATUS_STORM32LINK_PRESENT      =  6; #0x0200
...
sub ConvertStr2StatusBarStr{
  ...
  if( CheckStatus($s2,$STATUS_IMU_PRESENT) ){
    if( CheckStatus($s2,$STATUS_NTBUS_INUSE) ){
      $result .= ' @NTBUS';
    }else{
  ...
  if( CheckStatus($s2,$STATUS_IMU2_PRESENT) ){
    if( CheckStatus($s2,$STATUS_NTBUS_INUSE) ){
      ...
      if( CheckStatus($s2,$STATUS_IMU2_HIGHADR) ){
        $result .= ' @HIGH ADR = on-brd IMU'; }else{ $result .= ' @LOW ADR = ext. IMU'; }
...
#XX
  $result .= '    LINK:';
  if( CheckStatus($s2,$STATUS_STORM32LINK_PRESENT) ){ $result .= '  AVAIL'; }else{ $result .= ' none'; }
  $result .= '    BAT:';
  if( CheckStatus($s2,$STATUS_BAT_ISCONNECTED) ){ $result .= ' CONNECTED'; }else{ $result .= ' none'; }
  $result .= '    VOLTAGE:';
  if( CheckStatus($s2,$STATUS_BAT_VOLTAGEISLOW) ){ $result .= ' LOW'; }else{ $result .= ' OK'; }
...
#XX
  if( CheckStatus($s2,$STATUS_STORM32LINK_PRESENT) ){ TextOut("\r\n".'  STorM32-LINK is PRESENT'); }else{ TextOut("\r\n".'  STorM32-LINK is not available'); }
  my ($sss)= GetState( $data[0] ); #state
...
#XX  $w_DataDisplay->dd_CycleTime->Text( $ddData[$DataCycleTime_p].' us' );
  ...
  my $NTBusStatus= CheckStatus($status,$STATUS_NTBUS_INUSE);
  if( $NTBusStatus ){
    $w_DataDisplay->dd_I2CError_label->Text( 'Bus Errors' );
  }else{
    $w_DataDisplay->dd_I2CError_label->Text( 'I2c Errors' );
  }
...
#XX
  my $LinkStatus= CheckStatus($status,$STATUS_STORM32LINK_OK);
  my $LinkUsage= CheckStatus($status,$STATUS_STORM32LINK_INUSE);
  my $LinkPresent= CheckStatus($status,$STATUS_STORM32LINK_PRESENT);
  my $LinkFlag= $LinkStatus;
  if(( $LinkStatus>0 )and( $LinkUsage==0 )){ $LinkFlag= 3; }
  if( $LinkPresent==0 ){ $LinkFlag= 2; }
  $w_DataDisplay->dd_STorM32LinkStatus->Text( $STorM32LinkStatusText[$LinkFlag] );
  $w_DataDisplay->dd_STorM32LinkStatus->Change( -background => $STorM32LinkStatusColors[$LinkFlag] );
...
##  if( not OpenPort() ){ ClosePort(); $ConfigureGimbalTool_IsRunning= 0; goto WERROR; } ###this doesn't capture a broken connection!!! BUT it captures an invalig COM port !!!
...
##  if( not ConnectionIsValid() ){ ConnectToBoardwoRead(); }
  ...
  TextOut( ' ok' );
  #set Mavlink Configuration
#  TextOut( "\r\n".'Disable heartbeat...' );
#  $Option= $NameToOptionHash{'Mavlink Configuration'};
...
if($ConfigureGimbal_StepNr==1){
  ...
  while( $checkvoltageloop ){
    ...
    if( substr($s,length($s)-1,1) ne 'o' ){ goto WERROR };
    my @data = unpack( "v$CMD_d_PARAMETER_ZAHL", $s ); ####THIS IS A BUG; RIGHT?????
#XX    my $s2= UIntToBitstr( $data[1] ); #status
    my $s2= UIntToBitstr( $data[$DataStatus_p] ); #status
  ...
  while( $checkstatusloop ){
    ...
    my @data = unpack( "v$CMD_d_PARAMETER_ZAHL", $s ); ####THIS IS A BUG; RIGHT?????
#XX    my ($sss)= GetState( $data[0] ); #state
    my ($sss)= GetState( $data[$DataState_p] ); #state
    $w_ConfigureGimbal->configuregimbal_WelcomeText2->Text(
...
WIKILINK:
  ...
  $w_ConfigureGimbal->AddLabel( -name=> 'm_faq_Text2_link',
  -pos=> [$tx,$ty + 8*13],
